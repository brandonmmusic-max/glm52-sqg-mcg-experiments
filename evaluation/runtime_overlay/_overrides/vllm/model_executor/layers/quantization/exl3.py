...
extension for the small eager parity window. Generic MCG dense and
non-rank-sliced MoE checkpoints use the bit-faithful
``exllamav3_ext.exl3_gemm`` parity path. SQG-XOR-Cheb-T12 matrices use B12X's
native dense Trellis entry point. Every logical checkpoint matrix is dispatched
independently: vLLM's packed QKV and gate/up modules are not treated as one EXL3
matrix because each source matrix owns its Hadamard vectors and codebook marker.
...
_B12X_SQG_TRELLIS_LINEAR_API: Any | None = None
...
_R7_ROUTED_BASE_PREFIX_RE = re.compile(
...
def _load_b12x_sqg_trellis_linear() -> Any:
    """Resolve SQG support without replacing the production B12X package."""

    global _B12X_SQG_TRELLIS_LINEAR_API
    if _B12X_SQG_TRELLIS_LINEAR_API is not None:
        return _B12X_SQG_TRELLIS_LINEAR_API
    try:
        from b12x_sqg.gemm import trellis_linear
    except Exception as exc:
        raise RuntimeError(
            "SQG EXL3 requires the isolated b12x_sqg.gemm.trellis_linear "
            "runtime overlay."
        ) from exc
    _B12X_SQG_TRELLIS_LINEAR_API = trellis_linear
    return trellis_linear


def _canonical_r7_tensor_prefix(prefix: str) -> tuple[str, int] | None:
    """Normalize one exact routed projection prefix for metadata lookup."""

    match = _R7_ROUTED_BASE_PREFIX_RE.fullmatch(prefix)
    if match is None:
        return None
    layer_index = int(match.group("layer"))
    canonical = (
        f"layers.{layer_index}.mlp.experts.{int(match.group('expert'))}."
        f"{match.group('projection')}"
    )
    return canonical, layer_index
...
def _r7_fused_layer_allowlist() -> frozenset[int] | None:
    """Exact layer indices allowed to use the R7 fused MCG runtime.

    The optional allowlist makes a treatment checkpoint path-comparable with a
    preserved baseline when selected SQG layers cannot use the MCG-only fused
    kernel.  It prevents later MCG layers from silently taking the skipped
    layers' count-budget slots.
    """

    raw = os.environ.get("VLLM_EXL3_R7_FUSED_ALLOWLIST")
    if raw is None or raw == "":
        return None
    tokens = raw.split(",")
    if any(re.fullmatch(r"(?:0|[1-9][0-9]*)", token) is None for token in tokens):
        raise ValueError(
            "VLLM_EXL3_R7_FUSED_ALLOWLIST must be a canonical comma-separated "
            f"list of non-negative layer indices, got {raw!r}"
        )
    values = tuple(int(token) for token in tokens)
    if len(set(values)) != len(values):
        raise ValueError(
            "VLLM_EXL3_R7_FUSED_ALLOWLIST contains duplicate layer indices: "
            f"{raw!r}"
        )
    return frozenset(values)


_PRESERVED_R7_FUSED_LAYERS = frozenset({6, 7, 8, *range(10, 55)})
_FRESH_SQG_TREATMENT_LAYERS = frozenset({6, 28, 52, 77})


def _r7_native_mcg_control_layers() -> frozenset[int] | None:
    """Enable only the preregistered four-layer native-MCG control arm."""

    raw = os.environ.get("VLLM_EXL3_NATIVE_MCG_CONTROL_LAYERS")
    if raw is None or raw == "":
        return None
    tokens = raw.split(",")
    if any(re.fullmatch(r"(?:0|[1-9][0-9]*)", token) is None for token in tokens):
        raise ValueError(
            "VLLM_EXL3_NATIVE_MCG_CONTROL_LAYERS must be a canonical "
            f"comma-separated layer list, got {raw!r}"
        )
    values = frozenset(int(token) for token in tokens)
    if len(values) != len(tokens) or values != _FRESH_SQG_TREATMENT_LAYERS:
        raise ValueError(
            "native-MCG control is defined only for exact layers 6,28,52,77; "
            f"got {raw!r}"
        )
    return values


def _r7_expected_reserved_layers() -> frozenset[int] | None:
    """Expected preserved slots, or None when accounting is not requested."""

    raw = os.environ.get("VLLM_EXL3_R7_EXPECT_RESERVED_LAYERS")
    if raw is None or raw == "":
        return None
    if raw == "none":
        return frozenset()
    ...
    if any(re.fullmatch(r"(?:0|[1-9][0-9]*)", token) is None for token in tokens):
        raise ValueError(
            "VLLM_EXL3_R7_EXPECT_RESERVED_LAYERS must be `none` or a "
            f"canonical comma-separated layer list, got {raw!r}"
        )
    values = frozenset(int(token) for token in tokens)
    if len(values) != len(tokens) or not values.issubset(_PRESERVED_R7_FUSED_LAYERS):
        raise ValueError(f"invalid reserved-layer expectation: {raw!r}")
    return values


def _r7_layer_index_for_allowlist(layer: Any) -> int:
    name = str(getattr(layer, "layer_name", "") or getattr(layer, "prefix", ""))
    match = re.search(r"(?:^|\.)layers\.(\d+)(?:\.|$)", name)
    if match is None:
        raise ValueError(
            "cannot resolve layer index for VLLM_EXL3_R7_FUSED_ALLOWLIST from "
            f"{name!r}"
        )
    return int(match.group(1))


def _reserve_preserved_r7_fused_slot(quant_config: Any, layer: Any) -> int:
    """Charge one baseline fused slot without entering the MCG-only kernel."""

    allowlist = _r7_fused_layer_allowlist()
    if allowlist != _PRESERVED_R7_FUSED_LAYERS:
        raise RuntimeError(
            "preserved fused-slot reservation requires the exact historical "
            "48-layer allowlist"
        )
    layer_index = _r7_layer_index_for_allowlist(layer)
    if layer_index not in allowlist:
        raise RuntimeError(f"layer {layer_index} has no preserved fused slot")
    reserved = set(getattr(quant_config, "_r7_reserved_fused_layer_indices", ()))
    if layer_index in reserved:
        raise RuntimeError(f"layer {layer_index} attempted duplicate slot reservation")
    budget = _r7_fused_layer_budget()
    used = int(getattr(quant_config, "_r7_fused_layers", 0))
    if budget is not None and used >= budget:
        raise RuntimeError(
            "layer cannot reserve its preserved-baseline fused slot: "
            f"layer={layer_index} used={used} budget={budget}"
        )
    reserved.add(layer_index)
    quant_config._r7_reserved_fused_layer_indices = tuple(sorted(reserved))
    quant_config._r7_fused_layers = used + 1
    return used + 1


def _validate_preserved_r7_accounting(quant_config: Any, layer: Any) -> None:
    """Fail once the historical range closes unless fused accounting is exact."""

    expected = _r7_expected_reserved_layers()
    if expected is None:
        return
    allowlist = _r7_fused_layer_allowlist()
    if allowlist != _PRESERVED_R7_FUSED_LAYERS:
        raise RuntimeError("reserved-slot accounting requires exact fused allowlist")
    layer_index = _r7_layer_index_for_allowlist(layer)
    if layer_index <= max(allowlist):
        return
    used = int(getattr(quant_config, "_r7_fused_layers", 0))
    reserved = frozenset(
        getattr(quant_config, "_r7_reserved_fused_layer_indices", ())
    )
    if used != len(allowlist) or reserved != expected:
        raise RuntimeError(
            "preserved R7 fused accounting differs: "
            f"used={used}/{len(allowlist)} reserved={sorted(reserved)} "
            f"expected={sorted(expected)}"
        )
    if not getattr(quant_config, "_r7_fused_accounting_logged", False):
        logger.info(
            "R7 preserved fused accounting closed: actual=%d reserved=%d "
            "total=%d reserved_layers=%s",
            used - len(reserved),
            len(reserved),
            used,
            sorted(reserved),
        )
        quant_config._r7_fused_accounting_logged = True
...
def _b12x_sqg_trellis_weight(
    trellis: torch.Tensor,
    suh: torch.Tensor,
    svh: torch.Tensor,
    dtype: torch.dtype,
) -> Any:
    """Prepare SQG weights using only the isolated B12X runtime."""

    api = _load_b12x_sqg_trellis_linear()
    cache = getattr(trellis, "_vllm_b12x_sqg_prepared_weights", None)
    if cache is None:
        cache = {}
        trellis._vllm_b12x_sqg_prepared_weights = cache
    key = (id(suh), id(svh), dtype)
    weight = cache.get(key)
    if weight is None:
        weight = api.prepare_weight(
...
def _b12x_sqg_c_tmp_elements(rows: int, columns: int) -> int:
    """Conservative W4A16 scratch for SQG rates without the K6 fast path."""

    padded_rows = max(
        ((int(rows) + 47) // 48) * 48,
        ((int(rows) + 63) // 64) * 64,
    )
    return min(int(columns) * padded_rows, _B12X_TRELLIS_C_TMP_CAP)
...
@torch.library.custom_op(
    "vllm::b12x_sqg_trellis_linear_out",
    mutates_args=("output", "gemm_output", "c_tmp", "rotated_f16"),
    device_types="cuda",
)
def _b12x_sqg_trellis_linear_out(
    x: torch.Tensor,
    trellis: torch.Tensor,
    suh: torch.Tensor,
    svh: torch.Tensor,
    ...
    """Execute SQG-XOR-Cheb-T12 into caller-owned output and scratch."""

    api = _load_b12x_sqg_trellis_linear()
    weight = _b12x_sqg_trellis_weight(trellis, suh, svh, x.dtype)
    api.run(
        x,
        weight,
        output=output,
...
@_b12x_sqg_trellis_linear_out.register_fake
def _b12x_sqg_trellis_linear_out_fake(
    x: torch.Tensor,
    trellis: torch.Tensor,
    suh: torch.Tensor,
    svh: torch.Tensor,
...
def _b12x_sqg_trellis_linear(
    x: torch.Tensor,
    trellis: torch.Tensor,
    suh: torch.Tensor,
    svh: torch.Tensor,
) -> torch.Tensor:
    """Run one K2/K3/K4 SQG-XOR-Cheb-T12 matrix through B12X."""

    bits = int(trellis.shape[2]) // 16
    if bits not in (2, 3, 4):
        raise ValueError(
            "SQG-XOR-Cheb-T12 supports only K2/K3/K4 payloads, "
            f"got K{bits}"
        )
    output = torch.empty(
        (x.shape[0], trellis.shape[1] * 16), dtype=x.dtype, device=x.device
    ...
    gemm_output = torch.empty_like(output)
    c_tmp = torch.empty(
        (_b12x_sqg_c_tmp_elements(x.shape[0], output.shape[1]),),
        dtype=torch.float32,
        device=x.device,
    )
    rotated_f16 = torch.empty_like(x)
    _b12x_sqg_trellis_linear_out(
        x,
        trellis,
        suh,
        svh,
...
class Exl3Config(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> Exl3Config:
        ...
        if _r7 is not None:
            ...
            raw_codebook_overrides = _r7.get("codebook_overrides", {})
            if not isinstance(raw_codebook_overrides, dict):
                raise ValueError("R7 EXL3 codebook_overrides must be an object")
            codebook_overrides: dict[int, str] = {}
            for raw_layer, raw_codebook in raw_codebook_overrides.items():
                try:
                    layer_index = int(raw_layer)
                except (TypeError, ValueError):
                    raise ValueError(
                        "R7 EXL3 codebook override keys must be layer indices, "
                        f"got {raw_layer!r}"
                    ) from None
                if not int(layers[0]) <= layer_index <= int(layers[1]):
                    raise ValueError(
                        f"R7 EXL3 codebook override layer {layer_index} is outside "
                        f"moe_layers={list(layers)}"
                    )
                codebook = str(raw_codebook).strip().lower()
                if codebook not in {"mcg", "sqg_xor_cheb_t12"}:
                    raise ValueError(
                        "R7 EXL3 codebook overrides support only 'mcg' or "
                        f"'sqg_xor_cheb_t12', got {raw_codebook!r}"
                    )
                codebook_overrides[layer_index] = codebook
            raw_tensor_overrides = _r7.get("codebook_tensor_overrides", {})
            if not isinstance(raw_tensor_overrides, dict):
                raise ValueError(
                    "R7 EXL3 codebook_tensor_overrides must be an object"
                )
            codebook_tensor_overrides: dict[str, str] = {}
            for raw_prefix, raw_codebook in raw_tensor_overrides.items():
                if not isinstance(raw_prefix, str):
                    raise ValueError(
                        "R7 EXL3 tensor override keys must be routed projection "
                        f"prefixes, got {raw_prefix!r}"
                    )
                normalized = _canonical_r7_tensor_prefix(raw_prefix)
                if normalized is None:
                    raise ValueError(
                        "R7 EXL3 tensor override key must match "
                        "[model.]layers.L.mlp.experts.E."
                        f"{{gate_proj|up_proj|down_proj}}, got {raw_prefix!r}"
                    )
                canonical_prefix, layer_index = normalized
                if not int(layers[0]) <= layer_index <= int(layers[1]):
                    raise ValueError(
                        f"R7 EXL3 tensor override {raw_prefix!r} is outside "
                        f"moe_layers={list(layers)}"
                    )
                codebook = str(raw_codebook).strip().lower()
                if codebook not in {"mcg", "sqg_xor_cheb_t12"}:
                    raise ValueError(
                        "R7 EXL3 tensor overrides support only 'mcg' or "
                        f"'sqg_xor_cheb_t12', got {raw_codebook!r}"
                    )
                existing = codebook_tensor_overrides.get(canonical_prefix)
                if existing is not None and existing != codebook:
                    raise ValueError(
                        "R7 EXL3 tensor override aliases disagree for "
                        f"{canonical_prefix!r}: {existing!r} vs {codebook!r}"
                    )
                codebook_tensor_overrides[canonical_prefix] = codebook
            ...
            instance.r7_routed_experts = {
                **_r7,
                ...
                "codebook_tensor_overrides": codebook_tensor_overrides,
            }
        # --- end glm52-mdispatch ---
        return instance
    ...
    def _validate_storage_metadata(self) -> None:
        ...
        for prefix, entry in self.tensor_storage.items():
            ...
            markers = sorted(set(_CODEBOOK_MARKERS) & suffixes)
            if len(markers) > 1:
    ...
    def codebook_for_prefix(self, prefix: str) -> str | None:
        ...
        if entry is None:
            ...
            if self._r7_layer_range_contains(prefix):
                r7 = getattr(self, "r7_routed_experts", {})
                match = re.search(r"layers\.(\d+)\b", prefix)
                layer_index = None if match is None else int(match.group(1))
                normalized = _canonical_r7_tensor_prefix(prefix)
                tensor_overrides = r7.get("codebook_tensor_overrides", {})
                tensor_codebook = (
                    None
                    if normalized is None
                    else tensor_overrides.get(normalized[0])
                )
                overrides = r7.get("codebook_overrides", {})
                codebook = (
                    tensor_codebook
                    if tensor_codebook is not None
                    else overrides.get(layer_index, r7.get("codebook"))
                )
                if codebook in ("mcg", "mul1", "sqg_xor_cheb_t12"):
                    return str(codebook)
            # --- end glm52-mdispatch ---
            return None
...
class Exl3LinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        for shard_id in layer.exl3_shard_ids:
            ...
            expected_marker = (
                "sqg" if expected == "sqg_xor_cheb_t12" else expected
            )
            if len(present) > 1:
    ...
    def _apply_one(
        ...
        has_mcg = shard_id in layer.mcg.exl3_tensors
        has_mul1 = shard_id in layer.mul1.exl3_tensors
        has_sqg = shard_id in layer.sqg.exl3_tensors
        if has_sqg:
            output = _b12x_sqg_trellis_linear(
                x,
                trellis,
                layer.suh.exl3_tensors[shard_id],
                layer.svh.exl3_tensors[shard_id],
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        sqg_payloads = []
        for group in ("w13", "w2"):
            sqg_markers = getattr(layer, f"{group}_sqg").exl3_tensors
            trellis_tensors = getattr(layer, f"{group}_trellis").exl3_tensors
            sqg_payloads.extend(trellis_tensors[key] for key in sqg_markers)
        native_control = _r7_native_mcg_control_layers()
        native_control_layer = (
            _r7_layer_index_for_allowlist(layer)
            if native_control is not None
            else None
        )
        if (
            sqg_payloads
            and native_control is not None
            and native_control_layer in native_control
        ):
            raise RuntimeError(
                "native-MCG control layer contains SQG payloads: "
                f"{layer.layer_name}"
            )
        if sqg_payloads:
            invalid_bits = sorted(
                {
                    ...
                    for tensor in sqg_payloads
                    if int(tensor.shape[2]) // 16 not in (2, 3, 4)
                }
            )
            if invalid_bits:
                raise ValueError(
                    "SQG-XOR-Cheb-T12 routed experts support only K2/K3/K4; "
                    f"found {invalid_bits} in {layer.layer_name}"
                )
            # The R7 fused and extension paths decode MCG. Keep the converted
            # layer's native per-tensor topology and dispatch each routed
            # projection through B12X's dense SQG decoder instead.
            layer.exl3_r7_graph = False
            layer.exl3_r7_fused = False
            fused_allowlist = _r7_fused_layer_allowlist()
            if fused_allowlist is not None:
                layer_index = _r7_layer_index_for_allowlist(layer)
            if fused_allowlist is not None and layer_index in fused_allowlist:
                slot = _reserve_preserved_r7_fused_slot(self.quant_config, layer)
                logger.info(
                    "EXL3 SQG layer %s: reserving preserved-baseline R7 fused "
                    "slot %d so no later MCG layer can substitute",
                    layer.layer_name,
                    slot,
                )
            logger.info(
                "EXL3 SQG layer %s: retaining %d per-projection native tensors",
                layer.layer_name,
                len(sqg_payloads),
            )
            return

        if native_control is not None and native_control_layer in native_control:
            layer_index = int(native_control_layer)
            marker_keys = set()
            trellis_keys = set()
            for group in ("w13", "w2"):
                marker_keys.update(getattr(layer, f"{group}_mcg").exl3_tensors)
                trellis_keys.update(getattr(layer, f"{group}_trellis").exl3_tensors)
            expected_tensors = int(layer.local_num_experts) * 3
            if marker_keys != trellis_keys or len(marker_keys) != expected_tensors:
                raise RuntimeError(
                    "native-MCG control requires one exclusive MCG marker per "
                    f"trellis tensor in {layer.layer_name}: markers={len(marker_keys)} "
                    f"trellis={len(trellis_keys)} expected={expected_tensors}"
                )
            layer.exl3_r7_graph = False
            layer.exl3_r7_fused = False
            if layer_index in _PRESERVED_R7_FUSED_LAYERS:
                slot = _reserve_preserved_r7_fused_slot(self.quant_config, layer)
                logger.info(
                    "EXL3 native-MCG control layer %s: reserving preserved "
                    "R7 fused slot %d",
                    layer.layer_name,
                    slot,
                )
            logger.info(
                "EXL3 native-MCG control layer %s: retaining %d exclusive MCG "
                "tensors on native nonfused dispatch",
                layer.layer_name,
                len(marker_keys),
            )
            return
        ...
        for expert_id in range(layer.local_num_experts):
            for shard_id, projection in projections.items():
                ...
                present = [
                    ...
                    for marker in _CODEBOOK_MARKERS
                    if key
                    in getattr(layer, f"{group}_{marker}").exl3_tensors
                ]
                expected_marker = (
                    "sqg" if expected == "sqg_xor_cheb_t12" else expected
                )
                ...
                if expected_marker is not None and present != [expected_marker]:
                    raise ValueError(
                        f"EXL3 MoE {prefix} is missing {expected_marker}"
                    )
    ...
    def _prepare_r7_sparkinfer_weights(self, layer: RoutedExperts) -> bool:
        ...
        if fused_allowlist is not None:
            layer_index = _r7_layer_index_for_allowlist(layer)
            if layer_index not in fused_allowlist:
                _validate_preserved_r7_accounting(self.quant_config, layer)
                logger.info(
                    "R7 fused MoE %s: excluded by exact preserved-baseline "
                    "allowlist",
                    layer.layer_name,
                )
                return False
    ...
    def _apply_expert(
        ...
        if key in getattr(layer, f"{group}_sqg").exl3_tensors:
            output = _b12x_sqg_trellis_linear(x, trellis, suh, svh)
        else:
            output = _exl3_gemm(
                x,
                trellis,
