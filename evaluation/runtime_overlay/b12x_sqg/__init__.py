"""b12x_sqg — consumer-Blackwell (SM120/SM121) kernels.
...
collectives, ported from the b12x_sqg project.  One grammar everywhere:

- ops live at ``b12x_sqg.<group>.<op>`` and declare themselves via ``META``;
...
def find_op(qualname: str) -> OpMeta:
    ...
    if qualname not in _OPS:
        raise KeyError(
            f"unknown experimental b12x_sqg op {qualname!r}; known ops: {sorted(_OPS)}"
        )
