...
from b12x_sqg._lib.utils import cuda_stream_to_int
from b12x_sqg._lib.dense_gemm import (
    _WO_SPARK_MAX_SMS,
    dense_gemm,
...
def pack_mxfp8_scales_for_dense_gemm(
    ...
    *,
    ...
    """Pack compact MXFP8 row/chunk scales into b12x_sqg dense-GEMM MMA layout.
...
def _requantize_block_fp8_to_ue8m0(
    ...
    *,
    ...
    block_amax / 448` is an *arbitrary* fp32 128x128 block scale. The b12x_sqg MMA
...
def pack_wo_projection_fp8_block_scaled_weights_mxfp8(
    ...
    *,
    ...
    """Pack local DSV4 WO-A/WO-B checkpoint FP8 weights for the b12x_sqg WO path."""
...
@torch.library.custom_op(
    "b12x_sqg::quantize_wo_a_input_inv_rope_mxfp8_alloc",
    mutates_args=(),
)
...
@torch.library.custom_op(
    "b12x_sqg::quantize_wo_a_input_inv_rope_mxfp8_launch",
    ...
    *,
    ...
    if out is None:
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.b12x_sqg.quantize_wo_a_input_inv_rope_mxfp8_alloc(
                o,
                positions,
                cos_sin_cache,
                tokens,
    ...
    _check_mxfp8_rows_storage(out, m=tokens, k=group_width, num_groups=groups)
    torch.ops.b12x_sqg.quantize_wo_a_input_inv_rope_mxfp8_launch(
        o,
        positions,
        cos_sin_cache,
        out.values,
...
@torch.library.custom_op(
    "b12x_sqg::quantize_wo_b_input_mxfp8_alloc",
    mutates_args=(),
)
...
@torch.library.custom_op(
    "b12x_sqg::quantize_wo_b_input_mxfp8_launch",
    mutates_args=("out_values", "out_scale_rows", "out_scale_mma"),
)
...
def quantize_wo_b_input_mxfp8(
    ...
    *,
    ...
    if out is None:
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.b12x_sqg.quantize_wo_b_input_mxfp8_alloc(
                source_trg,
                tokens,
                rank,
                groups,
    ...
    torch.ops.b12x_sqg.quantize_wo_b_input_mxfp8_launch(
        source_trg,
        out.values,
        out.scale_rows,
        out.scale_mma,
...
@torch.library.custom_op(
    "b12x_sqg::wo_projection_inv_rope_mxfp8_fused",
    mutates_args=(),
)
...
def wo_projection_inv_rope_mxfp8(
    ...
    *,
    ...
    output = torch.ops.b12x_sqg.wo_projection_inv_rope_mxfp8_fused(
        o,
        positions,
        cos_sin_cache,
        weights.wo_a.values,
