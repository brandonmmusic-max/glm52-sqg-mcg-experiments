...
performs.  The FP4-path dequant primitives in ``b12x_sqg._lib.intrinsics``
...
@torch.library.custom_op("b12x_sqg::bmm_mxfp8", mutates_args=("out",))
def _op(
    lhs: torch.Tensor,
    b_values: torch.Tensor,
    b_scales: torch.Tensor,
...
@torch.library.custom_op(
    "b12x_sqg::mla_query_projection_mxfp8", mutates_args=("out",)
)
...
def mla_query_projection(
    ...
    *,
    ...
    if stream is not None:
        ...
        stream_int = int(_torch_stream(stream, b_values.device).cuda_stream)
    torch.ops.b12x_sqg.mla_query_projection_mxfp8(
        lhs,
        b_values,
        b_scales,
        q_pe,
