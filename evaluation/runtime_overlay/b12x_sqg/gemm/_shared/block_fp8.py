...
from b12x_sqg.gemm._shared.wo_mxfp8 import (
    MXFP8Rows,
    MXFP8_SCALE_K_TILE,
...
from b12x_sqg._lib.quant.mxfp8_rows import quantize_mxfp8_rows_cute
...
def _check_block_size(block_size: Sequence[int]) -> tuple[int, int]:
    ...
    if (block_n, block_k) != (128, 128):
        raise ValueError(
            f"b12x_sqg block FP8 linear currently supports 128x128 weight blocks, got {block_size}"
        )
...
def _c_dtype_name(dtype: torch.dtype) -> str:
    ...
    if dtype == torch.float16:
        return "float16"
    raise ValueError(
        f"b12x_sqg block FP8 linear output dtype must be bf16/fp16, got {dtype}"
    )
...
def pack_block_fp8_linear_weight_mxfp8(
    ...
    *,
    ...
    """Pack serialized block-FP8 linear weights for the native b12x_sqg MXFP8 GEMM.
...
@torch.library.custom_op(
    "b12x_sqg::quantize_block_fp8_linear_input_mxfp8_alloc",
    mutates_args=(),
)
    ...
    *,
    ...
    if out is None:
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.b12x_sqg.quantize_block_fp8_linear_input_mxfp8_alloc(
...
@torch.library.custom_op(
    "b12x_sqg::block_fp8_linear_mxfp8_fused",
    mutates_args=(),
)
...
def block_fp8_linear_mxfp8(
    ...
    *,
    ...
    """Run a serialized block-FP8 linear through the native b12x_sqg MXFP8 GEMM.
    ...
    if binding is None:
        ...
        output = torch.ops.b12x_sqg.block_fp8_linear_mxfp8_fused(
            source_2d,
