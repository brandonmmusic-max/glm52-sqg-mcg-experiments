...
from b12x_sqg.gemm._shared.block_fp8 import (
    quantize_block_fp8_linear_input_mxfp8,
)
from b12x_sqg._lib.dense_gemm import dense_gemm
...
class MXFP8LinearWeight:
    """ModelOpt-style MXFP8 linear weight packed for b12x_sqg dense GEMM."""
...
def _c_dtype_name(dtype: torch.dtype) -> str:
    ...
    if dtype == torch.float16:
        return "float16"
    raise ValueError(f"b12x_sqg MXFP8 linear output dtype must be bf16/fp16, got {dtype}")
...
def pack_mxfp8_linear_weight(
    ...
    ModelOpt MXFP8 checkpoints require only `K % 32 == 0`. The underlying b12x_sqg
...
def mxfp8_linear(
    ...
    *,
    ...
    """Run a ModelOpt MXFP8 linear through the native b12x_sqg dense GEMM path."""
    ...
    if tokens == 0:
        ...
        output = torch.ops.b12x_sqg.mxfp8_linear_fused(
            source_2d,
