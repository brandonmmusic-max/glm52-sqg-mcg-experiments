...
Example::
    ...
    torchrun --standalone --nproc-per-node=8 \
      -m b12x_sqg.comm.pcie.overlap_probe \
...
def dma_crossover_bytes(
    ...
    for index, (payload_bytes, decision) in enumerate(ordered):
        ...
        if len(tail) < consecutive_wins:
            break
        if decision.backend == "b12x_sqg-dma" and all(
            item[1].backend == "b12x_sqg-dma" for item in tail
        ):
...
class CollectiveOverlapProbe:
    def __init__(self, config: ProbeConfig) -> None:
        ...
        from b12x_sqg.comm.pcie import DmaAllReduce
    ...
    def _initialize_query_split(self) -> None:
        from b12x_sqg.attention.nsa_indexer.paged import (
            index_topk_fp8,
            prepare_paged_indexer_metadata,
        )
        from b12x_sqg.attention.nsa_indexer.scratch import (
        ...
        from b12x_sqg.attention.nsa_indexer.tiled_topk import run_row_topk
