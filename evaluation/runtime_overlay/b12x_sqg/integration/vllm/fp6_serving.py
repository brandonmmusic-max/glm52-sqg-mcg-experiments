"""Framework enablement layer for serving b12x_sqg MX-FP6 (W6A6/W6A8) checkpoints.

This is the reference surface a vLLM / SGLang quantization backend calls to run an
FP6 checkpoint produced by :mod:`b12x_sqg.quantization.mxfp6.fp6_safetensors_export`.
It is framework-agnostic (no vLLM/SGLang imports) so it can be wired into any
fork; :mod:`b12x_sqg.integration.vllm.plugin` is the concrete vLLM adapter.

Enablement is twofold (mirrors how the FP4 b12x_sqg path is selected framework-side):
...
* prepared experts from :func:`b12x_sqg.moe.fused_moe.prepare_weights` with
...
* weight from :func:`b12x_sqg.quantization.mxfp6.load_fp6_dense_checkpoint` as an
  :class:`~b12x_sqg.quantization.mxfp6.fp6_dense_weights.FP6DenseWeight`.

End-to-end flow
---------------

    pip install b12x_sqg
    python scripts/quantize_model_fp6.py --model <bf16 model> --out <fp6 model> --arch auto
    export B12X_ENABLE_FP6=1
    vllm serve <fp6 model>      # plugin detects W6A6 and calls b12x_sqg
"""
...
def is_b12x_fp6_checkpoint(config: Any) -> bool:
    """True iff ``config`` declares a b12x_sqg FP6 (modelopt + W6A6) quantization."""
...
class B12XFP6MoEMethod:
    """Reference routed-MoE method backed by :mod:`b12x_sqg.moe.fused_moe`.
...
class B12XFP6LinearMethod:
    ...
    def apply(
        ...
        from b12x_sqg.quantization.mxfp6 import dense_fp6_linear
...
def load_b12x_fp6_moe_methods(
    ...
    *,
    ...
    from b12x_sqg.moe import fused_moe
    from b12x_sqg.quantization.mxfp6 import load_fp6_moe_checkpoint
    ...
    for layer_idx, weights in layers.items():
        ...
        from b12x_sqg._lib.fp6 import unswizzle_mxfp6_scales
...
def load_b12x_fp6_linear_methods(
    ...
    *,
    ...
    from b12x_sqg.quantization.mxfp6 import load_fp6_dense_checkpoint
