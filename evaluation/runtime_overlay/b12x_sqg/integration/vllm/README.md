# b12x_sqg FP6 vLLM integration

Thin vLLM adapter for b12x_sqg MX-FP6 (W6A6/W6A8) checkpoints.  This mirrors
how the maintainer's private fork wires NVFP4: the glue lives in
``b12x_sqg/integration/`` and calls the public ``plan`` / ``bind`` / ``run``
APIs — b12x_sqg itself does not auto-load into vLLM.
...
```bash
pip install -e /path/to/b12x_sqg-fp6
```

### 2. Register the plugin entry point

Add to your vLLM fork's ``pyproject.toml`` (or use b12x_sqg's optional entry
point if you install b12x_sqg with vLLM present):
