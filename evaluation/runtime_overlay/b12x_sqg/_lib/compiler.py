...
# part of every disk-cache key.  Must resolve to b12x_sqg.
...
def _cute_compile_cache_dir() -> Path:
    ...
    if xdg_cache_home:
        return Path(xdg_cache_home) / "b12x_sqg" / "compile"
    return Path.home() / ".cache" / "b12x_sqg" / "compile"
...
def _format_cute_compile_stack() -> str:
    ...
    lines = ["[b12x_sqg cute.compile] python_stack (most recent call last):"]
...
def _call_cute_compile(
    ...
    *,
    ...
    print(
        f"[b12x_sqg cute.compile] compile-start number={compile_number} {details}",
        flush=True,
    )
    ...
    try:
        ...
        print(
            f"[b12x_sqg cute.compile] compile-failed number={compile_number} "
    ...
    with _COMPILE_PROGRESS_LOCK:
        ...
        total = _COMPILE_PROGRESS_TOTAL_SECONDS
    print(
        f"[b12x_sqg cute.compile] compile-done number={compile_number} "
        f"duration_s={elapsed:.3f} total_compile_s={total:.3f} {details}",
        flush=True,
    )
    return compiled
...
def _function_fingerprint(func: Any) -> tuple[str, str, str]:
    ...
    if module.startswith("b12x_sqg"):
        return module, qualname, f"b12x_sqg:{_b12x_package_fingerprint()}"
    try:
...
def _build_compile_manifest(
    ...
    manifest: dict[str, Any] = {
        "schema": "b12x_sqg._lib.compile_manifest.v3",
        "cache_key": cache_key,
        "cache_format": cache_format,
...
def _load_cute_compile_from_disk(cache_key: str):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(
            prefix="b12x_sqg-cute-cache-load-"
        ) as raw_stage:
