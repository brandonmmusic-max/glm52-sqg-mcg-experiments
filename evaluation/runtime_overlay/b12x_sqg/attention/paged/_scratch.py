...
by ``b12x_sqg.attention.paged._forward.paged_attention_forward``. Decode graph
...
from b12x_sqg.attention.paged.planner import (
    PagedPlan,
    PagedPlanBudget,
    create_paged_plan,
    infer_paged_mode,
    ...
    def update_decode_graph_replay_metadata_from_runtime_cache_seqlens(
        ...
        if not self._plan.split_kv:
            if int(self._plan.window_left) < 0:
                return self
            from b12x_sqg.attention.paged.graph_replay import (
...
class B12XPagedAttentionBinding:
    ...
    def run(self) -> tuple[torch.Tensor, torch.Tensor]:
        from b12x_sqg.attention.paged._forward import (
