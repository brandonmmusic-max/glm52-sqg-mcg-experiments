...
class _GLMRoutePackedW4A8TileLaunch(_GLMRoutePackedW4A8MixedProjectionLaunch):
    ...
    def _stage_ktile(
        ...
        for copy in cutlass.range_constexpr(2):
            ...
            if unit < Int32(total_units):
                ...
                global_k16 = (
                    kt * Int32(_V2_KTILE_K32 * 2) + k16_local
                )
                global_n16 = n_tile * Int32(n16_per_cta) + n16_local
                src_word = (
                    tensor_base
                    + (
                        Int64(global_k16) * Int64(self.size_n // 16)
                        + Int64(global_n16)
                    )
                    * Int64(8 * int(bits))
    ...
    def _decode_tile_at_base_shared(
        ...
        tile_word = (
            k16_local * Int32(_V2_TILE_N // 16) + n16_local
        ) * Int32(8 * int(bits))
    ...
    def _decode_k32_pair_shared(
        ...
        tile_word0 = (
            kb * Int32(2) * Int32(_V2_TILE_N // 16) + n16_local
        ) * Int32(8 * int(bits))
                ...
                for t in cutlass.range_constexpr(_V2_N8_PER_WARP // 2):
                    ...
                    for h in cutlass.range_constexpr(2):
                        ...
                        if cutlass.const_expr(h == 0):
                            ...
                            b0 = b0_hi
                            b1 = b1_hi
                        for blk in cutlass.range_constexpr(
                            self.blocks_per_cta
                        ):
                            frag = facc
                            d0, d1, d2, d3 = mxfp8_mma_m16n8k32_f32_e4m3(
...
def _selected_w4a8_kernel() -> str:
    ...
    if kernel not in ("m64n8", "m128n64"):
        raise ValueError(
            "B12X_GLM_W4A8_KERNEL must be 'm64n8' (one-warp) or 'm128n64'"
        )
    return kernel
