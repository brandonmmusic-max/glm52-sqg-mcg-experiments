...
@pytest.mark.parametrize(("bits", "expected_words"), [(3, 1536), (4, 2048)])
def test_uniform_w4a8_launch_preserves_native_payload_size(
    bits: int,
    expected_words: int,
) -> None:
    ...
    assert launch.trellis_words == expected_words
    assert launch.pair_kind is None
    assert launch.rate_axis is None


@pytest.mark.parametrize(
    ("m_tile_rows", "m_groups"), [(16, 1), (32, 2), (64, 4), (128, 8)]
)
def test_uniform_w4a8_large_m_tiles_reuse_decoded_fragments(
    m_tile_rows: int, m_groups: int
) -> None:
    launch = _TrellisW4A8DenseLaunch(
        size_k=128,
        size_n=128,
    ...
    assert launch.m_tile_rows == m_tile_rows
    assert launch.m_groups == m_groups


def test_uniform_w4a8_rejects_unsupported_large_m_tile() -> None:
    with pytest.raises(ValueError, match="M tile"):
        _TrellisW4A8DenseLaunch(
            size_k=128,
            size_n=128,
            trellis_bits=3,
...
@pytest.mark.parametrize(
    ("m", "expected"),
    [(1, 16), (31, 16), (32, 32), (63, 32), (64, 64), (127, 64), (128, 128)],
)
def test_uniform_w4a8_selects_largest_decode_reuse_tile(
    m: int, expected: int
) -> None:
    assert select_dense_w4a8_m_tile_rows(m) == expected


def test_uniform_w4a8_entry_rejects_pair_before_cuda() -> None:
    weight = SimpleNamespace(trellis_pair_kind="P33")

    with pytest.raises(ValueError, match="rejects P24/P33"):
        api.run_trellis256_uniform_dense_w4a8(torch.empty(0), weight)


def test_run_uniform_w4a8_delegates_without_changing_storage(monkeypatch) -> None:
    x = torch.empty(0)
    weight = SimpleNamespace(trellis_pair_kind=None)
    ...
    monkeypatch.setattr(api, "run_trellis256_uniform_dense_w4a8", fake_run)
    actual = trellis_linear.run_uniform_w4a8(x, weight, output=output)
...
def test_dense_uniform_e4m3_w4a8_matches_quantized_reference_and_captures(
    bits: int,
) -> None:
    """Exercise checkpoint-native K3/K4 addressing through the E4M3 MMA."""
    torch.manual_seed(0x4A80 + bits)
    device = torch.device("cuda", torch.cuda.current_device())
    trellis = torch.randint(
        -32768,
        32767,
    ...
    actual = trellis_linear.run_uniform_w4a8(x, weight, **kwargs).clone()
    torch.cuda.synchronize(device)

    expected = (_reference_mxfp8_rows(x) @ reference_weight.float()).to(
        torch.float16
    )
    torch.testing.assert_close(actual, expected, rtol=2.0e-3, atol=2.0e-2)
    ...
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        captured = trellis_linear.run_uniform_w4a8(x, weight, **kwargs)
    graph.replay()
    torch.cuda.synchronize(device)
    assert torch.equal(captured, actual)
...
@pytest.mark.skipif(not _sm12x_available(), reason="requires an SM120/SM121 GPU")
@pytest.mark.parametrize("bits", [3, 4])
@pytest.mark.parametrize("m_tile_rows", [32, 64, 128])
def test_dense_uniform_e4m3_w4a8_large_m_reuse_matches_reference(
    bits: int, m_tile_rows: int
) -> None:
    """Reuse one decoded B fragment across M groups, including a tail tile."""
    torch.manual_seed(0x4A90 + bits + m_tile_rows)
    device = torch.device("cuda", torch.cuda.current_device())
    trellis = torch.randint(
        -32768,
        32767,
...
@pytest.mark.skipif(not _sm12x_available(), reason="requires an SM120/SM121 GPU")
def test_glm_route_packed_mixed_k3_k4_projection_matches_reference_and_capture(
) -> None:
    """Keep independent expert rates while reusing B across packed M64 rows."""
    torch.manual_seed(0x5208A8)
    device = torch.device("cuda", torch.cuda.current_device())
    experts, size_k, size_n = 4, 128, 128
    tokens, topk = 37, 3
    rates = (3, 4, 3, 4)
    pools: dict[int, list[torch.Tensor]] = {3: [], 4: []}
    references: list[torch.Tensor] = []
    slots = {
        3: torch.full((experts,), -1, dtype=torch.int32, device=device),
        4: torch.full((experts,), -1, dtype=torch.int32, device=device),
    }
    for expert, bits in enumerate(rates):
        trellis = torch.randint(
            -32768,
            32767,
            (size_k // 16, size_n // 16, 16 * bits),
            dtype=torch.int16,
            device=device,
        )
        slots[bits][expert] = len(pools[bits])
        pools[bits].append(trellis)
        references.append(
            _reconstruct_native(
                trellis, codebook="sqg_xor_cheb_t12"
            ).to(device)
    ...
    prepared = GLMRoutePackedW4A8Projection(
        ...
        size_n=size_n,
        num_experts=experts,
        trellis_k3=torch.stack(pools[3]).contiguous(),
        trellis_k4=torch.stack(pools[4]).contiguous(),
        expert_slots_k3=slots[3],
        expert_slots_k4=slots[4],
    )

    source = (torch.randn((tokens, size_k), device=device) * 0.125).half()
    quantized = empty_mxfp8_rows_for_dense_gemm(tokens, size_k, device=device)
    ...
    flat_routes = torch.arange(tokens * topk, device=device)
    topk_ids = ((flat_routes * 5 + flat_routes // topk) % experts).to(
        torch.int32
    ).reshape(tokens, topk)
    packed_routes, block_experts, _packed_count = pack_topk_routes_by_expert(
        topk_ids,
        128,
        experts,
    )
    output = torch.empty(
        (tokens * topk, size_n), dtype=torch.float16, device=device
    )
    actual = run_glm_route_packed_w4a8_projection(
        quantized,
        prepared,
        packed_routes,
        block_experts,
    ...
    source_q = _reference_mxfp8_rows(source)
    expected = torch.stack(
        [
            source_q[route // topk] @ references[int(topk_ids.reshape(-1)[route])].float()
            for route in range(tokens * topk)
        ]
    ).half()
    torch.testing.assert_close(actual, expected, rtol=2.0e-3, atol=2.0e-2)
    ...
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        captured = run_glm_route_packed_w4a8_projection(
            quantized,
            prepared,
            packed_routes,
            block_experts,
...
@pytest.mark.skipif(not _sm12x_available(), reason="requires an SM120/SM121 GPU")
def test_glm_gate_up_transform_uses_exact_silu_and_captures() -> None:
    """GLM must use SiLU(gate)*up, never the Kimi SiTU contract."""
    torch.manual_seed(0x52051)
    device = torch.device("cuda", torch.cuda.current_device())
    experts, routes, width = 4, 11, 256
    gate = (torch.randn((routes, width), device=device) * 0.25).half()
    up = (torch.randn_like(gate) * 0.25).half()
    route_experts = torch.tensor(
        [0, 3, 1, 2, 3, 0, 2, 1, 1, 3, 0],
        dtype=torch.int32,
        device=device,
    )
    gate_svh = (
        0.75 + 0.5 * torch.rand((experts, width), device=device)
    ).half()
    up_svh = (
        0.75 + 0.5 * torch.rand((experts, width), device=device)
    ).half()
    ...
    ones = torch.ones((width,), dtype=torch.float16, device=device)

    actual = run_glm_gate_up_output_transform_silu(
        gate,
        up,
        route_experts,
        gate_svh,
    ...
    indices = route_experts.long()
    gate_ref = _hadamard128_reference(gate).float() * gate_svh[indices].float()
    up_ref = _hadamard128_reference(up).float() * up_svh[indices].float()
    expected = (torch.nn.functional.silu(gate_ref) * up_ref).half()
    torch.testing.assert_close(actual, expected, rtol=3.0e-3, atol=3.0e-3)

    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        captured = run_glm_gate_up_output_transform_silu(
            gate,
            up,
            route_experts,
            gate_svh,
...
@pytest.mark.skipif(not _sm12x_available(), reason="requires an SM120/SM121 GPU")
def test_glm_route_packed_mixed_k3_k4_w4a16_matches_reference_and_capture(
) -> None:
    """The hybrid down path must retain independent rates and compact bytes."""
    torch.manual_seed(0x520A16)
    device = torch.device("cuda", torch.cuda.current_device())
    experts, size_k, size_n = 4, 128, 128
    tokens, topk = 37, 3
    routes = tokens * topk
    ...
    runtime = prepare_glm_route_packed_w4a16_runtime(
        prepared, routes=routes, topk=topk
    )
    source = (torch.randn((tokens, size_k), device=device) * 0.125).half()
    route_experts = (
        (torch.arange(routes, device=device) * 7 + 1) % experts
    ).to(torch.int32)
    output = torch.empty((routes, size_n), dtype=torch.float16, device=device)
    actual = run_glm_route_packed_w4a16_projection(
        source, route_experts, prepared, output, runtime
    ).clone()
    torch.cuda.synchronize(device)

    expected = torch.stack(
        [
            source[route // topk].float()
            @ references[int(route_experts[route])].float()
            for route in range(routes)
        ]
    ).half()
    for expert in range(experts):
        mask = torch.tensor(
            [int(route_expert) == expert for route_expert in route_experts.cpu()],
            dtype=torch.bool,
            device=device,
        )
        torch.testing.assert_close(
            actual[mask], expected[mask], rtol=3.0e-3, atol=3.0e-2
        )

    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        captured = run_glm_route_packed_w4a16_projection(
            source, route_experts, prepared, output, runtime
        )
    graph.replay()
    torch.cuda.synchronize(device)
...
@pytest.mark.skipif(not _sm12x_available(), reason="requires an SM120/SM121 GPU")
@pytest.mark.parametrize("blocks_per_cta", (8, 4))
def test_glm_route_packed_w4a8_tile_kernel_bit_matches_one_warp(
    monkeypatch: pytest.MonkeyPatch,
    blocks_per_cta: int,
) -> None:
    """The M64xN256 tile kernel must reproduce the one-warp kernel bit-exactly.

    Both kernels accumulate the same K32 MMA chain in the same order over the
    same operands, so their FP16 outputs must be identical, not merely close.
    The tile kernel is additionally checked against the dense reference and
    for CUDA-graph capture stability.
    """
    torch.manual_seed(0x5208B8)
    device = torch.device("cuda", torch.cuda.current_device())
    experts, size_k, size_n = 5, 256, 512
    tokens, topk = 53, 3
    rates = (3, 4, 4, 3, 3)
    pools: dict[int, list[torch.Tensor]] = {3: [], 4: []}
    references: list[torch.Tensor] = []
    ...
    for expert, bits in enumerate(rates):
        ...
        pools[bits].append(trellis)
        references.append(
            _reconstruct_native(trellis, codebook="sqg_xor_cheb_t12").to(device)
        )
    prepared = GLMRoutePackedW4A8Projection(
        size_k=size_k,
        size_n=size_n,
    ...
    flat_routes = torch.arange(tokens * topk, device=device)
    topk_ids = ((flat_routes * 7 + flat_routes // topk) % experts).to(
        torch.int32
    ).reshape(tokens, topk)
    packed_routes, block_experts, _packed_count = pack_topk_routes_by_expert(
        topk_ids,
    ...
    monkeypatch.setenv("B12X_GLM_W4A8_KERNEL", "m64n8")
    baseline = run_glm_route_packed_w4a8_projection(
        quantized,
        prepared,
        packed_routes,
        block_experts,
    ...
    monkeypatch.setenv("B12X_GLM_W4A8_KERNEL", "m128n64")
    monkeypatch.setenv("B12X_GLM_W4A8_V2_BLOCKS", str(blocks_per_cta))
    actual = run_glm_route_packed_w4a8_projection(
        quantized,
        prepared,
        packed_routes,
    ...
    assert torch.equal(actual, baseline), "tile kernel diverged from one-warp"

    source_q = _reference_mxfp8_rows(source)
    expected = torch.stack(
        [
            source_q[route // topk]
            @ references[int(topk_ids.reshape(-1)[route])].float()
            for route in range(tokens * topk)
        ]
    ).half()
    torch.testing.assert_close(actual, expected, rtol=2.0e-3, atol=2.0e-2)
