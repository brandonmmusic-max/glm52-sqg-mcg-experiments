    ...
    def __init__(
        ...
        *,
        ...
        if self.route_block_rows != _GLM_ROUTE_BLOCK_ROWS:
            raise ValueError(
                "the initial GLM route-packed W4A8 kernel requires M64 blocks"
            )
    ...
    def __call__(
        ...
        values = cute.make_tensor(
            ...
            cute.make_ordered_layout(
                (input_rows, self.size_k // 4), order=(1, 0)
            ),
        )
        scale_rows = cute.make_tensor(
            scale_rows_ptr,
            cute.make_ordered_layout(
                (input_rows, self.size_k // 32), order=(1, 0)
            ),
        )
        trellis = cute.make_tensor(
            trellis_ptr,
            cute.make_layout(
                (Int64(pool_experts) * Int64(self.trellis_words),)
            ),
        )
...
class _GLMRoutePackedW4A8MixedProjectionLaunch(
    _GLMRoutePackedW4A8ProjectionLaunch
):
    """Dispatch K3/K4 once per expert-homogeneous route block.

    The first implementation launched both rate-specialized grids across every
    block and let the non-owning grid skip decode.  At GLM prefill sizes those
    inactive CTAs were material.  This launch keeps both native pools but
    selects the owning decoder inside one grid, preserving independent tensor
    rates without paying two full block schedules.
    """
    ...
    def __call__(
        ...
        trellis_k3 = cute.make_tensor(
            trellis_k3_ptr,
            cute.make_layout(
                (Int64(pool_experts_k3) * Int64(self.trellis_words_k3),)
            ),
        )
        trellis_k4 = cute.make_tensor(
            trellis_k4_ptr,
            cute.make_layout(
                (Int64(pool_experts_k4) * Int64(self.trellis_words_k4),)
            ),
        )
        rank_lut = cute.make_tensor(rank_lut_ptr, cute.make_layout((4096,)))
        packed_route_indices = cute.make_tensor(
...
class _GLMRoutePackedW4A8TileLaunch(_GLMRoutePackedW4A8MixedProjectionLaunch):
    ...
    def __init__(
        ...
        *,
        ...
        if int(blocks_per_cta) not in (2, 4):
            raise ValueError("v2 blocks_per_cta must be 2 or 4")
        if self.size_n % _V2_TILE_N:
            raise ValueError("v2 requires size_n divisible by 256")
        ...
        # M64 blocks are split across (4 // blocks_per_cta) CTA parts.
        self.m_parts = 4 // self.blocks_per_cta
        self.a_words = self.blocks_per_cta * _V2_A_BLOCK_WORDS
        self.asf_words = self.blocks_per_cta * 16
        self.stage_words = self.a_words + self.asf_words
        ...
        class Storage:
            sData: cute.struct.Align[
                cute.struct.MemRange[
                    cutlass.Uint32, _V2_STAGES * self.stage_words
                ],
                16,
            ]
    ...
    def _stage_ktile(
        ...
        """cp.async one (A, Asf) K128 tile into ``stage_base``.
    ...
    def _tile_body(
        ...
        packed_base = (
            block * Int32(self.route_block_rows)
            + part * Int32(self.blocks_per_cta * 16)
        )
        ...
        a_src_words = cute.make_rmem_tensor(
            (self.blocks_per_cta,), cutlass.Int32
        )
        for blk in cutlass.range_constexpr(self.blocks_per_cta):
            src_words = Int32(-1)
        ...
        facc = cute.make_rmem_tensor(
            (self.blocks_per_cta, _V2_N8_PER_WARP, 4), Float32
        )
        facc.fill(0.0)

        # ---- cp.async double-buffer: prefetch k-tile 0. ----
        self._stage_ktile(
            values,
            scale_rows,
            s_base,
        ...
        while kt < k_tiles:
            ...
            if nxt < k_tiles:
                self._stage_ktile(
                    values,
                    scale_rows,
                    s_base + (nxt & Int32(1)) * Int32(self.stage_bytes),
                    tid,
                    nxt,
                    a_src_words,
                    a_dst_addr,
            ...
            cur = s_base + (kt & Int32(1)) * Int32(self.stage_bytes)
            ...
            asc = cute.make_rmem_tensor(
                (self.blocks_per_cta,), cutlass.Uint32
            )
            for blk in cutlass.range_constexpr(self.blocks_per_cta):
        ...
        for blk in cutlass.range_constexpr(self.blocks_per_cta):
            ...
            for i in cutlass.range_constexpr(_V2_N8_PER_WARP):
                ...
                if route_lo >= Int32(0) and route_lo < routes:
                    ...
                    output[route_lo, col + Int32(1)] = cutlass.Float16(
                        facc[blk, i, 1]
                    )
                if route_hi >= Int32(0) and route_hi < routes:
                    output[route_hi, col] = cutlass.Float16(facc[blk, i, 2])
                    output[route_hi, col + Int32(1)] = cutlass.Float16(
                        facc[blk, i, 3]
                    )
...
def _selected_w4a8_kernel() -> str:
    kernel = os.environ.get("B12X_GLM_W4A8_KERNEL", "m64n256").strip().lower()
    if kernel not in ("m64n8", "m64n256"):
        raise ValueError(
            "B12X_GLM_W4A8_KERNEL must be 'm64n8' (one-warp) or 'm64n256'"
        )
    return kernel


def _v2_blocks_per_cta() -> int:
    blocks = int(os.environ.get("B12X_GLM_W4A8_V2_BLOCKS", "4"))
    if blocks not in (2, 4):
        raise ValueError("B12X_GLM_W4A8_V2_BLOCKS must be 2 or 4")
    return blocks
...
def run_glm_route_packed_w4a8_projection(
    ...
    *,
    ...
    # The tile kernel requires N256/K128 closure; smaller legacy shapes keep
    # the one-warp kernel so existing callers stay valid.
    use_tile_kernel = (
        _selected_w4a8_kernel() == "m64n256"
