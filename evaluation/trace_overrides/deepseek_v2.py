...
def _sqg_tail_trace_layers() -> set[int]:
    raw = os.getenv("SQG_TAIL_TRACE_LAYERS", "6,28,52,77")
    return {int(value.strip()) for value in raw.split(",") if value.strip()}


def _sqg_tail_trace_rank() -> int:
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        return int(torch.distributed.get_rank())
    return int(os.getenv("RANK", "0"))


def _sqg_tail_trace_enabled(layer: int | None) -> bool:
    return (
        os.getenv("SQG_TAIL_TRACE", "0") == "1"
        and layer is not None
        and layer in _sqg_tail_trace_layers()
        and bool(os.getenv("SQG_TAIL_TRACE_DIR"))
    )


def _sqg_tail_trace_save(
    *,
    layer: int,
    invocation: int,
    ...
    if int(hidden.shape[0]) <= 1:
        return
    tensors = {
        "positions": positions.detach().to(device="cpu", dtype=torch.int32).contiguous(),
        "hidden": hidden.detach().to(device="cpu", dtype=torch.bfloat16).contiguous(),
        "router_logits": router_logits.detach().to(device="cpu", dtype=torch.float32).contiguous(),
        "topk_weights": topk_weights.detach().to(device="cpu", dtype=torch.float32).contiguous(),
        "topk_ids": topk_ids.detach().to(device="cpu", dtype=torch.int16).contiguous(),
        "moe_output": moe_output.detach().to(device="cpu", dtype=torch.bfloat16).contiguous(),
        "residual_before_moe": residual.detach().to(device="cpu", dtype=torch.bfloat16).contiguous(),
        "residual_stream_after_moe": (moe_output + residual)
        .detach()
        .to(device="cpu", dtype=torch.bfloat16)
        .contiguous(),
    }
    rows = int(hidden.shape[0])
    if any(int(value.shape[0]) != rows for value in tensors.values()):
        raise RuntimeError("SQG tail trace tensor rows do not align")
    root = Path(os.environ["SQG_TAIL_TRACE_DIR"])
    root.mkdir(parents=True, exist_ok=True)
    rank = _sqg_tail_trace_rank()
    destination = root / (
        f"layer-{layer:03d}-rank-{rank:03d}-call-{invocation:03d}.safetensors"
    )
    partial = destination.with_suffix(destination.suffix + ".partial")
    if destination.exists() or partial.exists():
        raise RuntimeError(f"SQG tail trace destination already exists: {destination}")
    save_file(
        tensors,
        partial,
        metadata={
...
class DeepseekV2MoE(nn.Module):
    def __init__(
        ...
        self._sqg_tail_trace_layer = (
            int(layer_match.group(1)) if layer_match is not None else None
        )
        self._sqg_tail_trace_invocation = 0
        self._sqg_tail_trace_router_result = None
        self._sqg_tail_trace_positions = None
        self._sqg_tail_trace_residual = None
        ...
        if _sqg_tail_trace_enabled(self._sqg_tail_trace_layer):
            router = getattr(self.experts, "router", None)
            original_select = getattr(router, "select_experts", None)
            if router is None or not callable(original_select):
                raise RuntimeError("SQG tail trace could not find the live MoE router")

            def traced_select(router_self, *args, **kwargs):
                result = original_select(*args, **kwargs)
                if not isinstance(result, tuple) or len(result) < 2:
                    raise RuntimeError("SQG tail trace router result schema changed")
                self._sqg_tail_trace_router_result = (
                    result[0].detach(),
                    result[1].detach(),
                )
                return result

            router.select_experts = types.MethodType(traced_select, router)
    ...
    def forward(
        ...
        local_final_hidden_states = final_hidden_states
        if _sqg_tail_trace_enabled(self._sqg_tail_trace_layer):
            captured = self._sqg_tail_trace_router_result
            positions = self._sqg_tail_trace_positions
            residual = self._sqg_tail_trace_residual
            if captured is None or positions is None or residual is None:
                raise RuntimeError("SQG tail trace inputs or router result are absent")
            topk_weights, topk_ids = captured
            rows = int(hidden_states.shape[0])
            expected_rows = (
                int(positions.shape[0]),
                int(residual.shape[0]),
                int(router_logits.shape[0]),
                int(topk_weights.shape[0]),
                int(topk_ids.shape[0]),
                int(local_final_hidden_states.shape[0]),
            )
            if any(value != rows for value in expected_rows):
                raise RuntimeError(
                    f"SQG tail trace row mismatch: hidden={rows}, others={expected_rows}"
                )
            _sqg_tail_trace_save(
                layer=int(self._sqg_tail_trace_layer),
                invocation=self._sqg_tail_trace_invocation,
                positions=positions,
                hidden=hidden_states,
                router_logits=router_logits,
                topk_weights=topk_weights,
                topk_ids=topk_ids,
                moe_output=local_final_hidden_states,
                residual=residual,
            )
            self._sqg_tail_trace_invocation += 1
            self._sqg_tail_trace_router_result = None
...
class DeepseekV2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            and _sqg_tail_trace_enabled(self.mlp._sqg_tail_trace_layer)
        ):
            trace_positions = positions
            if int(trace_positions.shape[0]) != int(hidden_states.shape[0]):
                world_size = get_tensor_model_parallel_world_size()
                rank = get_tensor_model_parallel_rank()
                position_pad = (-int(trace_positions.shape[0])) % world_size
                trace_positions = torch.nn.functional.pad(
                    trace_positions, (0, position_pad), value=-1
                ).chunk(world_size, dim=0)[rank]
            if int(trace_positions.shape[0]) != int(hidden_states.shape[0]):
                raise RuntimeError("SQG tail trace positions do not match MoE rows")
            self.mlp._sqg_tail_trace_positions = trace_positions
            self.mlp._sqg_tail_trace_residual = residual
        if self.use_sequence_parallel_moe:
