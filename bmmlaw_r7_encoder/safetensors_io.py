...
def torch_tensor_entry(name: str, tensor) -> TensorEntry:
    ...
    # PyTorch rejects a dtype-changing view directly on a rank-zero tensor
    # when the element sizes differ.  Flatten only for byte extraction while
    # retaining the original shape in the Safetensors header, so scalar marker
    # tensors remain true shape-[] scalars on round trip.
