...
class SequentialWalk:
    ...
    def _run_layer(self, layer: int) -> None:
        ...
        if self.journal.has(encode_stage):
            ...
            if not self.journal.has(capture_stage):
            ...
            # Reopen deterministic routing sidecars on resume and compare their
            # hashes with the immutable capture stage.
            capture = self.backend.open_capture(layer=layer, routing_dir=routing_dir)
            ...
            oracle_result = audit_v2_layer(
                manifest_path, codec=self.codec, tp_sizes=(16,)
            )
