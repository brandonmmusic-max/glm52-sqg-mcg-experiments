...
class LayerProcessor:
    ...
    def _record_cold(self, record: dict[str, object]) -> None:
        key = sha256_bytes(canonical_json_bytes(record))
        self.cold_audit.setdefault(key, record)
        ...
        *,
        ...
        for rows in self.backend.iter_expert_rows(
            ...
            inputs = down_inputs_from_roundtrip(
                hidden,
                gate.reconstructed_kn.to(self.device),
                up.reconstructed_kn.to(self.device),
            )
            if inputs.shape[0]:
    ...
    def run(
        ...
        *,
            ...
            if len(self.codecs) > 1:
                ...
                for _records in self._warm_experts_parallel(
                    ...
                    for _record in sorted(
                        _records,
                        key=lambda r: (r.tensor_id.key, int(r.bits)),
                    ):
                        ledger.add(_record)
            for expert in range(NUM_EXPERTS):
                expert_keys = tuple(
        ...
        for expert in range(NUM_EXPERTS):
            ...
            install_record = self.backend.install_encoded_expert(
                layer=capture.layer,
                expert=expert,
                encoded={
                    item.tensor_id.key: {
        ...
        official_layer_audit = self.backend.audit_installed_layer(layer=capture.layer)
