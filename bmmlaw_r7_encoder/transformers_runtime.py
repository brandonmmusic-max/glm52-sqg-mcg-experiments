...
class _CarrierLoader:
    ...
    def __init__(self, root: Path, inventory: Mapping[str, object], codec) -> None:
        self.root = root
    ...
    def tensor(self, name: str):
        ...
        if info is None or info.payload.sha256() != record["payload_sha256"]:
...
class TransformersSequentialRuntime(GLM52Runtime):
    ...
    def __init__(self, config) -> None:
        ...
        if (
            load_runtime_code_inventory(config.runtime_inventory)["inventory_sha256"]
            != self._fingerprint
        ):
        ...
        self.loader = _CarrierLoader(
            config.carrier.resolve(), self.carrier_inventory, self._get_codec()
        )
    ...
    def _get_codec(self) -> Exl3TrellisCodec:
        if self._codec is None:
            numeric = load_numeric_environment(self.owner_config.numeric_inventory)
    ...
    def install_encoded_expert(
        ...
        for key, record in sorted(encoded.items()):
            ...
            supplied = (
                torch.as_tensor(record["reconstructed_kn"])
                .detach()
                .half()
                .contiguous()
                .cpu()
            ...
            if _tensor_sha256(decoded) != record[
                "reconstruction_sha256"
            ] or not torch.equal(decoded, supplied):
            ...
            installed[projection] = supplied.to(torch.bfloat16)
        ...
        *,
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=len(modules)) as pool:
                for shard, metadata, out, tk in pool.map(
                    _compute, enumerate(pending[1:])
                ):
                    yield self._write_state(
                        output_partial=output_partial,
                        layer_input=layer + 1,
