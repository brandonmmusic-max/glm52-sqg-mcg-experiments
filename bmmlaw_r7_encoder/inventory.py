...
def load_numeric_environment(path: str | Path) -> dict[str, object]:
    configure_deterministic_environment()
    payload = read_json(path)
    digest = payload.pop("inventory_sha256", None)
    if digest != sha256_bytes(canonical_json_bytes(payload)):
        raise ValueError("numeric environment seal mismatch")
    if sha256_file(payload["numeric_core"]) != payload["numeric_core_sha256"]:  # type: ignore[arg-type]
        raise ValueError("numeric core changed after inventory")
    if sha256_file(payload["extension"]) != payload["extension_sha256"]:  # type: ignore[arg-type]
...
def load_runtime_code_inventory(path: str | Path) -> dict[str, object]:
    payload = read_json(path)
    digest = payload.pop("inventory_sha256", None)
    if digest != sha256_bytes(canonical_json_bytes(payload)):
