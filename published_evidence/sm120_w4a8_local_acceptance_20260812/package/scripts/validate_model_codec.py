...
Gates covered (host-side, CPU, zero third-party deps):
  * quantization_config.json contract fields (full_w4a8, census, down targets
    for layers 3..78, r7 routed contract, K6 non-routed contract, no A16).
  ...
  * Routed census from shard headers: for every routed layer 3..78 and every
    ...
    == sealed 384/384 per layer, no MCG/mul1 marker anywhere, all trellis
    int16, all rotation vectors fp16 with matching extents, and the
    intermediate axis is 128*TP4-divisible.
...
def main() -> None:
    ...
    if moe_intermediate % (128 * 4):
        problems.append(
            f"moe_intermediate_size {moe_intermediate} not divisible by 512; "
            "TP4 slices would not be 128-aligned"
        )
    ...
    if census != {"k3": 384, "k4": 384, "total": 768}:
    ...
    for layer in range(3, 79):
        ...
        if counts[3] != 384 or counts[4] != 384 or sum(counts.values()) != 768:
            problems.append(f"layer {layer} census {dict(counts)} != 384/384")
    if routed_missing:
    ...
    receipt = {
        ...
        "per_layer_census_ok": all(
            per_layer[l][3] == 384 and per_layer[l][4] == 384 for l in per_layer
        ),
