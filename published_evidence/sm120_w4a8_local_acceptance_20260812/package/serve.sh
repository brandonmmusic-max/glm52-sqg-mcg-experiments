...
PROJECT=glm52-sqg-w4a8-sm120-local
...
MODEL_REVISION_PIN=593dd0d2de6f79ce4e65303930c22c75e1359d44
...
verify_model_revision() {
  log "validating model directory against pinned revision ${MODEL_REVISION_PIN}"
  python3 "${SCRIPT_DIR}/scripts/validate_model_codec.py" \
    --model "${MODEL_DIR}" \
    --revision "${MODEL_REVISION_PIN}" \
    --quick \
...
server_running() {
  docker ps --filter "name=glm52-sqg-w4a8-sm120-accept" --filter status=running -q | grep -q . || return 1
}
...
cmd_start() {
  ...
  log "starting acceptance server (MTP0, enforce-eager, BF16 KV) on ${BIND_ADDRESS:-127.0.0.1}:${HOST_PORT:-9418}"
...
cmd_status() {
  "${COMPOSE[@]}" ps
  local cid
  cid="$(docker ps -q --filter name=glm52-sqg-w4a8-sm120)"
  if [[ -n "${cid}" ]]; then
...
cmd_logs() {
  "${COMPOSE[@]}" logs "${@}" server 2>/dev/null || docker logs "${@}" glm52-sqg-w4a8-sm120-accept
}
...
cmd_verify() {
  python3 "${SCRIPT_DIR}/scripts/verify_runtime_path.py" \
    --container glm52-sqg-w4a8-sm120-accept \
...
cmd_kld() {
  ...
  log "KLD receipt: ${RESULTS_DIR}/kld/kld_sm120_tp4dcp4.json"
  if (( was_running )); then
