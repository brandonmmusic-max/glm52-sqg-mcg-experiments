...
preserved byte-for-byte; only the engine topology (TP1/PP4/DCP1 on four SM120
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--routed-experts-npz",
        help=(
            "Optionally capture the exact prompt routing matrix and write it "
            "as a compressed NPZ. This is diagnostic only and does not alter "
            "the sealed KLD calculation."
        ),
    )
    parser.add_argument(
        "--routes-only",
        action="store_true",
        help=(
            "Capture exact routed experts without requesting full-vocabulary "
            "prompt logprobs. This preserves the sealed prompt and topology "
            "but avoids the KLD profiling workspace; --routed-experts-npz is required."
        ),
    )
    ...
    if args.routes_only and not args.routed_experts_npz:
        parser.error("--routes-only requires --routed-experts-npz")
    topology = (
    ...
    expected_attestation = allowed_topologies.get(topology)
    if expected_attestation is None:
        parser.error(
            "coupled SM120 KLD runner requires DCP1 with TP1/PP4 or TP4/PP1"
        )
    ...
    args.topology_attestation = expected_attestation
    return args
...
def shutdown_llm(llm: LLM) -> None:
    """Stop the multiprocess engine before Python begins finalization.

    Relying on the engine client's weakref finalizer races CPython shutdown in
    this long-lived CUDA process and can abort after the atomic result has
    already been written.  An explicit, generous shutdown keeps the result and
    process exit status consistent.
    """
    llm.llm_engine.engine_core.shutdown(timeout=60.0)


def exit_after_sealed_result(llm: LLM) -> None:
    """Exit cleanly after the atomic receipt is durable.

    Some CUDA/native worker threads survive the engine shutdown long enough to
    race CPython finalization and turn a valid, fsynced KLD receipt into exit
    status 134.  The result is already complete at both call sites below, so
    flush the human-readable receipt and bypass only interpreter teardown.
    """
    shutdown_llm(llm)
    sys.stdout.flush()
    sys.stderr.flush()
...
def main() -> None:
    ...
    kwargs: dict[str, Any] = {
        ...
        **(
        ...
        "enable_return_routed_experts": bool(args.routed_experts_npz),
    }
    if not args.routes_only:
        kwargs["max_logprobs"] = -1
    if args.tokenizer:
    ...
    routed_experts_receipt: dict[str, Any] | None = None
    if args.routed_experts_npz:
        completion = output.outputs[0]
        routed = completion.routed_experts
        if routed is None:
            raise RuntimeError("vLLM returned no routed experts")
        routed_array = np.asarray(routed)
        if routed_array.ndim != 3 or routed_array.shape[0] != len(token_ids):
            raise RuntimeError(
                "routed experts shape differs: "
                f"{list(routed_array.shape)}; expected "
                f"[{len(token_ids)}, layers, topk]"
            )
        routes_path = Path(args.routed_experts_npz).resolve()
        routes_path.parent.mkdir(parents=True, exist_ok=True)
        routes_tmp = routes_path.with_name(f".{routes_path.name}.tmp-{os.getpid()}")
        with routes_tmp.open("xb") as handle:
            np.savez_compressed(handle, routed_experts=routed_array)
            handle.flush()
            os.fsync(handle.fileno())
        os.replace(routes_tmp, routes_path)
        routed_experts_receipt = {
            "path": str(routes_path),
            "sha256": sha256_file(routes_path),
            "shape": list(map(int, routed_array.shape)),
            "dtype": str(routed_array.dtype),
            "prompt_start": 0,
            "interpretation": (
                "row i contains routes used while processing prompt token i; "
                "KLD position i predicts prompt token i+1"
            ),
        }
    ...
    if args.routes_only:
        ...
        result = {
            "schema": (
                f"glm52-routed-experts-sm120-{args.topology_attestation}-v1"
            ),
            "complete": True,
            "routes_only": True,
            ...
            "routed_experts": routed_experts_receipt,
    ...
    result = {
        "schema": (
            f"glm52-bf16-reference-kld-sm120-{args.topology_attestation}-v1"
        ),
        "complete": True,
        "model": str(Path(args.model).resolve()),
        "model_revision": os.environ.get("MODEL_REVISION_PIN", ""),
