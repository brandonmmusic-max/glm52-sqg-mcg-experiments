    ...
    def forward_impl(
        ...
        if num_mqa_tokens > 0:
            ...
            if self.impl.dcp_world_size > 1 and not ckv_gather_used:
                ...
                assert self.dcp_manager is not None
                # Some sparse metadata classes (e.g. B12xMLASparseMetadata)
                # carry flat seq_lens/num_decodes instead of a decode
                # sub-struct; the else-arm below is written for exactly that
                # shape, so probe with getattr rather than assuming the
                # attribute exists.
                decode_view = getattr(attn_metadata, "decode", None)
                seq_lens = (
                    decode_view.seq_lens
                    if decode_view is not None
