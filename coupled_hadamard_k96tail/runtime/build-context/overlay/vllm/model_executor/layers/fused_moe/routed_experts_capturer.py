...
def get_routed_experts_attn_gid(kv_cache_config: KVCacheConfig) -> int:
    """Return an attention KV cache group used to key routed experts.

    Uniform-type grouping wraps per-layer attention specs in
    :class:`UniformTypeKVCacheSpecs`.  Such a group still has one shared block
    table and slot mapping, which is exactly the identity needed by routed
    expert capture.  Inspect the inner specs instead of rejecting the wrapper.
    """
    for gid, group in enumerate(kv_cache_config.kv_cache_groups):
        spec = group.kv_cache_spec
        if isinstance(spec, FullAttentionSpec):
            return gid
        if isinstance(spec, UniformTypeKVCacheSpecs) and any(
            isinstance(inner, FullAttentionSpec)
            for inner in spec.kv_cache_specs.values()
        ):
            return gid
    raise ValueError(
        "Routed-experts capture requires a full-attention KV cache group "
        "(directly or inside UniformTypeKVCacheSpecs)."
    )
