...
from .w4a8 import run_trellis256_uniform_dense_w4a8
from . import META
...
def run_sqg_k6_w6a16(
    ...
    *,
    ...
    """Execute one native SQG K6 matrix through the W6A16 kernel.

    This is deliberately a separate fail-closed endpoint.  It consumes the
    checkpoint-native K6 stream, decodes SQG labels with the contiguous
    64-bit stream rule, and performs A16 MMA.  MCG markers, non-K6 rates,
    pair containers, and FP16-oriented SQG variants are not admitted.
    """
    if str(getattr(weight, "trellis_codebook", "")).lower() != (
        "sqg_xor_cheb_t12"
    ):
        raise ValueError(
            "native SQG K6 W6A16 requires codebook='sqg_xor_cheb_t12'"
        )
    if int(getattr(weight, "trellis_bits", 0)) != 6:
        raise ValueError("native SQG K6 W6A16 requires an exact K6 payload")
    if getattr(weight, "trellis_pair_kind", None) is not None:
        raise ValueError("native SQG K6 W6A16 rejects pair-container payloads")
    if getattr(weight, "mcg", None) is not None:
        raise ValueError("native SQG K6 W6A16 forbids MCG marker lineage")
    if getattr(weight, "mul1_e4m3", None) is not None:
        raise ValueError("native SQG K6 W6A16 forbids mul1/FP16 codebooks")
    return run_trellis256_dense(
        x,
        weight,
        output=output,
...
def run_uniform_w4a8(
    x: torch.Tensor,
    weight: PreparedWeight,
    **kwargs,
) -> torch.Tensor:
    """Execute one native uniform K3/K4 SQG tensor through SM120 W4A8."""
    return run_trellis256_uniform_dense_w4a8(x, weight, **kwargs)
