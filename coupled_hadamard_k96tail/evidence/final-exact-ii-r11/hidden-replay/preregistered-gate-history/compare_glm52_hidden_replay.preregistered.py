...
def main() -> None:
    ...
    parser.add_argument("--max-baseline-position-delta", type=float, default=5e-4)
    ...
    baseline_position_delta = float(np.max(np.abs(position_kld - baseline_positions)))
    ...
    result = {
        "schema": "glm52-hidden-replay-one-context-kld-v1",
        "complete": True,
