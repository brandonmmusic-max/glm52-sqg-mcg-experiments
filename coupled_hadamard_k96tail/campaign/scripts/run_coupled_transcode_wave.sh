...
OUTPUT_ROOT=${OUTPUT_ROOT:-/media/brandonmusic/nvme1n1p3/glm52-coupled-3p0625-no-shortcut-work/$wave}
DETACH=${DETACH:-0}
GPU_BASE=${GPU_BASE:-0}
GPU_SPAN=${GPU_SPAN:-4}
...
ENCODE_START=${ENCODE_START:-0}
ENCODE_END=${ENCODE_END:-256}
ENCODE_SHARDS_PER_LAYER=${ENCODE_SHARDS_PER_LAYER:-8}
ENCODE_THREADS_PER_SHARD=${ENCODE_THREADS_PER_SHARD:-3}
for value in "$ENCODE_START" "$ENCODE_END" "$ENCODE_SHARDS_PER_LAYER" \
  "$ENCODE_THREADS_PER_SHARD"; do
  [[ "$value" =~ ^[0-9]+$ ]] || die "encode range, shard, and thread values must be integers"
done
((ENCODE_START >= 0 && ENCODE_START < ENCODE_END && ENCODE_END <= 256)) || \
  die "encode range must satisfy 0 <= ENCODE_START < ENCODE_END <= 256"
((ENCODE_SHARDS_PER_LAYER >= 1 && ENCODE_SHARDS_PER_LAYER <= 8 && \
   ENCODE_SHARDS_PER_LAYER <= ENCODE_END - ENCODE_START)) || \
  die "ENCODE_SHARDS_PER_LAYER must fit the selected expert range"
((ENCODE_THREADS_PER_SHARD >= 1 && ENCODE_THREADS_PER_SHARD <= 16)) || \
  die "ENCODE_THREADS_PER_SHARD must lie within 1..16"
...
for path in "$PROJECT_ROOT" "$PREPARATION_ROOT" "$BINDING_ROOT" "$CAPTURE_ROOT" \
  "$PROFILE_ROOT" "$SOURCE_SQG_ROOT" "$ALLOCATION_ROOT" "$QSRT_ROOT" \
...
for layer in "${run_layers[@]}"; do
  ...
  allocation="$ALLOCATION_ROOT/layer_${padded}${ALLOCATION_SUFFIX}"
...
for layer in "${run_layers[@]}"; do
  ...
  encode_span=$((ENCODE_END - ENCODE_START))
  for ((encode_shard = 0; encode_shard < ENCODE_SHARDS_PER_LAYER; encode_shard++)); do
    if ((GPU_SPAN <= 4)); then
      gpu=$((GPU_BASE + layer - start_layer))
    else
      gpu=$((GPU_BASE + ((layer - start_layer) * ENCODE_SHARDS_PER_LAYER + encode_shard) % GPU_SPAN))
    fi
    shard_start=$((ENCODE_START + encode_span * encode_shard / ENCODE_SHARDS_PER_LAYER))
    shard_end=$((ENCODE_START + encode_span * (encode_shard + 1) / ENCODE_SHARDS_PER_LAYER))
    name="glm52-goal019ffa7c-coupled-v4-${wave}-l${layer}-s${encode_shard}-r${BASHPID}"
    names+=("$name")
    lifecycle=(--rm)
    log_path="$OUTPUT_ROOT/logs/encode-l${padded}-${shard_start}-${shard_end}.log"
    if [[ "$DETACH" == 1 ]]; then
      lifecycle=(-d)
      log_path="${log_path}.launch-${BASHPID}"
    fi
    docker run "${lifecycle[@]}" --name "$name" --network none --gpus "device=$gpu" \
    --shm-size 16g --cpus "$ENCODE_THREADS_PER_SHARD" \
    ...
    -e "OMP_NUM_THREADS=$ENCODE_THREADS_PER_SHARD" \
    -e "MKL_NUM_THREADS=$ENCODE_THREADS_PER_SHARD" \
    -e "OPENBLAS_NUM_THREADS=$ENCODE_THREADS_PER_SHARD" \
    -e "NUMEXPR_NUM_THREADS=$ENCODE_THREADS_PER_SHARD" \
    ...
    scripts/encode_coupled_mixed_rate_shard.py \
      --preflight /output/preflight.json \
      --output-root /candidate --qsrt-root /qsrt \
      ...
      --layer "$layer" --start "$shard_start" --end "$shard_end" --draws 0 6 \
      --local-alpha 0.25 --device cuda:0 \
      --threads "$ENCODE_THREADS_PER_SHARD" --chunk-rows 256 \
    >"$log_path" 2>&1 &
    pids+=("$!")
  done
done
...
((failed == 0)) || exit 1
if [[ "$DETACH" == 1 ]]; then
  printf 'coupled wave encode launched detached: wave=%s output=%s containers=%s\n' \
    "$wave" "$OUTPUT_ROOT" "${names[*]}"
else
  printf 'coupled wave encode complete: wave=%s output=%s\n' "$wave" "$OUTPUT_ROOT"
fi
