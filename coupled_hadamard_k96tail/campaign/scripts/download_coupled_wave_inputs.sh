...
((end - start == 3 && start >= 3 && end <= 78)) || \
  die "wave must contain four consecutive routed layers"
...
includes=(
  --include "derived/wave_preflights/$wave/*"
  --include "capture_view/capture_manifest.json"
)
