...
def gate_up_transform_points(
    ...
    # The route-packed runtime materializes the H128 output in FP16 before
    # MXFP8 row quantization.  Keeping this boundary is required because QDQ
    # is nonlinear at adjacent FP16 values.
    post_h = _fp16_boundary(_block_hadamard_right(pre_h, hadamard))
...
def down_transform_points(
    ...
    # Fable's accepted GLM path stores SwiGLU, the down scale result, and the
    # H128 result in FP16 buffers before the MXFP8 endpoint.
    raw = _fp16_boundary(activation)
    pre_h = _fp16_boundary(raw * suh.float())
    post_h = _fp16_boundary(_block_hadamard_right(pre_h, hadamard))
    return {"raw": raw, "after_suh_pre_h": pre_h, "post_h_qdq_input": post_h}
...
def apply_output_transform(
    ...
    return _fp16_boundary(
        _block_hadamard_right(accumulated_fp16, hadamard) * svh.float()
    )


def apply_gate_up_output_transform_silu(
    gate_accumulated_fp16: torch.Tensor,
    up_accumulated_fp16: torch.Tensor,
    gate_svh: torch.Tensor,
    up_svh: torch.Tensor,
    hadamard: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Mirror the accepted route-packed GLM FC1 epilogue and SwiGLU buffers."""

    if gate_accumulated_fp16.shape != up_accumulated_fp16.shape:
        raise ValueError("gate/up accumulated outputs must be aligned")
    width = gate_accumulated_fp16.shape[1]
    if gate_svh.numel() != width or up_svh.numel() != width:
        raise ValueError("gate/up output scales do not match the intermediate width")
    gate_h = _fp16_boundary(
        _block_hadamard_right(gate_accumulated_fp16, hadamard)
    )
    up_h = _fp16_boundary(_block_hadamard_right(up_accumulated_fp16, hadamard))
    gate_output = gate_h.float() * gate_svh.float()
    up_output = up_h.float() * up_svh.float()
    activation = _fp16_boundary(F.silu(gate_output) * up_output)
    return gate_output, up_output, activation
...
def evaluate_sqg_arms(
    ...
    for name, operand in (("a16", h_a16), ("h_a8", h_a8)):
        ...
        gate_out, up_out, activation = apply_gate_up_output_transform_silu(
            gate_acc,
            up_acc,
            gate.svh,
            up.svh,
