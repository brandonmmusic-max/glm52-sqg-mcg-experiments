...
from .glm52_fresh_sqg.reference import payload_sha256, tensor_sha256
...
class LayerCaptureView:
    ...
    def load_hidden(
        ...
        *,
        ...
        bf16 = _torch_from_bf16_words(self.hidden_words[row_indices]).contiguous()
        target = torch.device(device)
        if target.type == "cuda":
            bf16 = bf16.pin_memory()
            return bf16.to(
                device=target, dtype=dtype, non_blocking=True
            ).contiguous()
        return bf16.to(device=target, dtype=dtype).contiguous()

    def iter_hidden_prefetch(
        self,
        row_indices: np.ndarray,
        *,
        chunk_rows: int,
        ...
        """Yield double-buffered hidden chunks from host mmap to one CUDA GPU."""

        if row_indices.ndim != 1 or chunk_rows <= 0:
            raise ValueError("prefetch rows must be one-dimensional and chunked")
        target = torch.device(device)
        if target.type != "cuda":
            for begin in range(0, row_indices.size, chunk_rows):
                end = min(row_indices.size, begin + chunk_rows)
                yield begin, end, self.load_hidden(
                    row_indices[begin:end], device=target, dtype=dtype
                )
            return
        streams = (torch.cuda.Stream(device=target), torch.cuda.Stream(device=target))

        def schedule(begin: int, stream_index: int):
            end = min(row_indices.size, begin + chunk_rows)
            words = _torch_from_bf16_words(
                self.hidden_words[row_indices[begin:end]]
            ).contiguous().pin_memory()
            with torch.cuda.stream(streams[stream_index]):
                tensor = words.to(
                    device=target, dtype=dtype, non_blocking=True
                ).contiguous()
            return begin, end, words, tensor, streams[stream_index]

        if row_indices.size == 0:
            return
        current = schedule(0, 0)
        next_begin = current[1]
        stream_index = 1
        while current is not None:
            following = (
                schedule(next_begin, stream_index)
                if next_begin < row_indices.size
                else None
            )
            begin, end, _host, tensor, copy_stream = current
            compute_stream = torch.cuda.current_stream(target)
            compute_stream.wait_stream(copy_stream)
            tensor.record_stream(compute_stream)
            yield begin, end, tensor
            current = following
            if following is not None:
                next_begin = following[1]
    ...
    def role_gate_square_mass_by_expert(
        self,
        role: str,
        *,
        ...
        result = torch.zeros(
            NUM_EXPERTS, dtype=torch.float64, device=numerical_device
        )
        mask = self.role_mask(role)
        ids = torch.from_numpy(np.array(self.topk_ids[mask], copy=True)).to(
            device=numerical_device, dtype=torch.int64
        )
        gates = torch.from_numpy(
            np.array(self.topk_weights[mask], dtype=np.float64, copy=True)
        ).to(numerical_device)
        result.scatter_add_(0, ids.flatten(), gates.square().flatten())
        if not bool(torch.isfinite(result).all()) or not bool(result.sum() > 0):
...
def build_layer_h13(
    ...
    *,
    ...
    for begin, end in _chunks(capture.rows, chunk_rows):
        ...
        denominator.add_(row_weights.double().sum())
        selected_rows += int(x.shape[0])
    if selected_rows <= 0 or not bool(torch.isfinite(denominator)) or not bool(
        denominator > 0
    ):
        raise ValueError("fit capture cannot construct a nonempty H13")
    covariance = accumulator.div_(denominator.to(accumulator_dtype))
    covariance = ((covariance + covariance.T) * 0.5).float().contiguous()
...
def derive_expert_permutation(
    ...
    *,
    ...
    evidence_seed = {
        ...
        "source_gate_payload_sha256": payload_sha256(gate_hf),
        "source_up_payload_sha256": payload_sha256(up_hf),
    }
...
class ProfileScaleEvidence:
    ...
    def families(self) -> dict[str, tuple[torch.Tensor, torch.Tensor]]:
        # Profiles are durable evidence, so reductions must not vary with the
        # assigned GPU architecture.  The saved FP32 scale tensors are the
        # authority; canonicalize them to CPU before means and normalization.
        gate_base = self.gate_input_base.detach().to(
            device="cpu", dtype=torch.float32
        )
        down_base = self.down_output_base.detach().to(
            device="cpu", dtype=torch.float32
        )
        gate_quarter = self.gate_block_quarter.detach().to(
            device="cpu", dtype=torch.float32
        )
        down_quarter = self.down_block_quarter.detach().to(
            device="cpu", dtype=torch.float32
        )
        gate_adjust = expand_blocks(gate_quarter, HIDDEN)
        down_adjust = expand_blocks(down_quarter, HIDDEN)
        ...
        return {
            ...
            "identity": (
                torch.ones_like(gate_base) * gate_reference,
                torch.ones_like(down_base),
            ),
            "aggregate_rms": (
                absolute_gate(gate_base),
                relative(down_base),
            ),
            "quarter_rms": (
                absolute_gate(gate_base * gate_adjust),
                relative(down_base * down_adjust),
            ),
            "inverse_quarter_rms": (
                absolute_gate(gate_base / gate_adjust),
                relative(down_base / down_adjust),
...
def build_profile_scale_evidence(
    ...
    *,
    ...
    if numerical_device.type != "cuda":
        raise ValueError("production profile-scale construction requires CUDA")
    masses = capture.role_gate_square_mass_by_expert(
        "fit", device=numerical_device
    )
    total_mass = masses.sum()
    gate_energy = torch.zeros(
        HIDDEN, dtype=torch.float64, device=numerical_device
    )
    down_energy = torch.zeros(
        HIDDEN, dtype=torch.float64, device=numerical_device
    )
    for expert in range(NUM_EXPERTS):
        weights = source.load_expert_bf16(
            capture.layer, expert, device=numerical_device
        )
        mass = masses[expert]
        if not bool(mass > 0):
            raise ValueError(
    ...
    def quarter_scales(values: torch.Tensor) -> torch.Tensor:
        cleaned = values.float().clamp_min(1e-20)
        geometric = cleaned.log().mean().exp()
        scales = (geometric / cleaned).pow(0.25).clamp(0.5, 2.0)
        return (scales / scales.log().mean().exp()).contiguous()

    gate_quarter = quarter_scales(h13_blocks)
    down_quarter = quarter_scales(down_blocks)
    evidence = {
        "construction": PROFILE_SCALE_CONSTRUCTION,
        "role": "fit",
        "layer": capture.layer,
        "fit_gate_square_mass_sha256": tensor_sha256(masses),
        "fit_gate_square_mass_sum": float(total_mass),
...
def make_shared_profiles(
    *,
    ...
    # Profiles are durable, caller-owned metadata.  Profile-scale evidence is
    # accumulated on the assigned CUDA device in production, while the stable
    # SHA-derived sign factory intentionally returns CPU tensors.  Canonicalize
    # both operands to CPU before constructing the profile so manifest hashing,
    # conceptual FP16 realization, and profile serialization never perform a
    # mixed-device expression.  The codec moves the sealed profile to the
    # assigned CUDA device exactly once via realize_shared_residual_profile().
    gate_signs = gate_signs.detach().to(device="cpu", dtype=torch.float32).contiguous()
    down_signs = down_signs.detach().to(device="cpu", dtype=torch.float32).contiguous()
    gate_scales = gate_scales.detach().to(device="cpu", dtype=torch.float32).contiguous()
    down_scales = down_scales.detach().to(device="cpu", dtype=torch.float32).contiguous()
