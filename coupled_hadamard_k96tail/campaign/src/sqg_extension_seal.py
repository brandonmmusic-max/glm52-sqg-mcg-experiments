...
SM103_SQG_EXTENSION_SEAL_SCHEMA = (
    "glm52-b300-sm103-kquant-sqg-prebuilt-extension-v1"
)
...
def validate_sqg_extension_seal(
    ...
    *,
    ...
    if value.get("schema") == SM103_SQG_EXTENSION_SEAL_SCHEMA:
        extension = value.get("extension")
        smoke = value.get("smoke")
        if (
            value.get("complete") is not True
            or value.get("passed") is not True
            or value.get("compute_capability") != [10, 3]
            or value.get("module_name") != SQG_EXTENSION_MODULE
            or value.get("build_contract") != SM103_BUILD_CONTRACT
            ...
            or value.get("kquant_extension") != extension
            or not isinstance(smoke, Mapping)
        ):
            raise ValueError("SM103 SQG extension construction seal differs")
        extension_path = Path(str(extension.get("path")))
        if (
            not extension_path.is_absolute()
            or extension_path.name != _expected_extension_name()
        ):
            raise ValueError("SM103 SQG extension runtime path/name differs")
        _regular(extension_path, label="sealed SM103 SQG extension")
        digest = sha256_file(extension_path)
        if (
            extension.get("sha256") != digest
            or extension.get("bytes") != extension_path.stat().st_size
        ):
            raise ValueError("sealed SM103 SQG extension bytes differ")
        ...
        if (
            ...
            or [record.get("bits") for record in records] != [2, 3, 4, 5, 6]
            or any(
                record.get("golden_match") is not True
                or record.get("cyclic_state_exact") is not True
                or not isinstance(record.get("output_sha256"), str)
                or not isinstance(record.get("indices_sha256"), str)
                for record in records
            )
        ):
            raise ValueError("SM103 SQG extension K2-K6 smoke receipt differs")
        if require_runtime_environment:
            if _runtime_environment() != value.get("runtime_environment"):
                raise ValueError("SM103 SQG extension runtime environment differs")
