...
REFERENCE_ROUTE_SCHEMA = "glm52-fused-live-route-admissibility-v3"
REFERENCE_ROUTE_DESCRIPTION = (
    "independent 0.5*torch.tanh(0.5*actual_router_logits)+0.5 matching the "
    "fused CUDA sigmoid form; preserve the exact live "
...
def validate_fused_layer_audit(value: object) -> dict:
    ...
    if value.get("schema") == "glm52-progressive-w4a8-layer-execution-audit-v1":
        encoded = value.get("encoded_w4a8_layers")
        unchanged = value.get("unchanged_mcg_layers")
        if (
            not isinstance(encoded, list)
            or not encoded
            or encoded != list(range(3, encoded[-1] + 1))
            or unchanged != sorted(set(ROUTED_LAYERS) - set(encoded))
            or value.get("routed_layers") != list(ROUTED_LAYERS)
            or value.get("w4a8_count") != len(encoded)
            or value.get("mcg_count") != len(unchanged)
            or value.get("allow_a16_fallback") is not False
            or value.get("construction_only") is not True
            or value.get("final_release_eligible") is not False
            or value.get("acceptance_eligible") is not False
            or value.get("selected_layer_modes")
            != {
                str(layer): "native_full_w4a8" if layer in encoded else "mcg"
                for layer in SELECTED_LAYERS
            }
        ):
            raise ValueError(f"post-load progressive W4A8 audit differs: {value}")
        digest = value.get("construction_manifest_sha256")
        if (
            not isinstance(digest, str)
            or len(digest) != 64
            or any(char not in "0123456789abcdef" for char in digest)
        ):
            raise ValueError("progressive W4A8 audit manifest hash differs")
        return dict(value)
...
def validate_router_audit(value: object, *, layer: int) -> dict:
    ...
    if execution_mode == "progressive_native_full_w4a8":
        observed_sources = value.get("class_sources")
        if not isinstance(observed_sources, dict):
            raise ValueError(f"layer {layer}: progressive class sources are absent")
        class_sources = {
            role: dict(record) for role, record in RUNTIME_CLASS_SOURCES.items()
        }
        quant = observed_sources.get("quant_method")
        if (
            not isinstance(quant, dict)
            or quant.get("class") != class_sources["quant_method"]["class"]
            or quant.get("path") != class_sources["quant_method"]["path"]
            or not isinstance(quant.get("sha256"), str)
            or len(quant["sha256"]) != 64
        ):
            raise ValueError(f"layer {layer}: progressive EXL3 class source differs")
        class_sources["quant_method"]["sha256"] = quant["sha256"]
    elif execution_mode not in (None, "sealed_all_mcg_teacher"):
        raise ValueError(f"layer {layer}: unknown execution mode")
    exact = {
        "layer": int(layer),
...
def validate_runtime_provenance_evidence(value: object) -> dict:
    ...
    observed_environment = value.get("environment")
    expected_environment = dict(RUNTIME_ENV)
    progressive = (
        isinstance(observed_environment, dict)
        and observed_environment.get("B12X_MOE_FORCE_A16") == "0"
    )
    if progressive:
        expected_environment["B12X_MOE_FORCE_A16"] = "0"
    exact = {
    ...
    b12x_tree = "/opt/venv/lib/python3.12/site-packages/b12x"
    expected_files = set(RUNTIME_FILES) | ({b12x_tree} if progressive else set())
    ...
    for filename, expected_sha256 in RUNTIME_FILES.items():
        ...
        is_progressive_exl3 = progressive and filename.endswith(
            "/vllm/model_executor/layers/quantization/exl3.py"
        )
        if (
            not isinstance(evidence, dict)
            or (
                not is_progressive_exl3
    ...
    if progressive:
        evidence = files[b12x_tree]
        if (
            not isinstance(evidence, dict)
            or evidence.get("kind") != "canonical_python_tree"
            or not isinstance(evidence.get("sha256"), str)
            or len(evidence["sha256"]) != 64
        ):
            raise ValueError("progressive B12X runtime provenance differs")
    return dict(value)
...
def write_layer_manifest(
    ...
    *,
    ...
    partial_replay = worker_result.get("partial_replay")
    if not isinstance(partial_replay, dict) or partial_replay.get("schema") != (
        "fresh-sqg-retained-hidden-router-replay-v1"
    ):
        raise ValueError(f"layer {layer}: partial replay evidence differs")
    replay_enabled = bool(partial_replay.get("enabled"))
    replay_rows = int(partial_replay.get("resume_rows", -1))
    replay_documents = int(partial_replay.get("resume_documents", -1))
    rerouted_rows = int(partial_replay.get("retained_hidden_rerouted_rows", -1))
    rerouted_calls = int(partial_replay.get("retained_hidden_rerouted_calls", -1))
    compared = int(partial_replay.get("live_hidden_compared_elements", -1))
    mismatched = int(partial_replay.get("live_hidden_mismatched_elements", -1))
    mismatched_calls = int(partial_replay.get("live_hidden_mismatched_calls", -1))
    if replay_enabled:
        if not (
            0 < replay_rows < OWNER_TOKENS
            and 0 < replay_documents < OWNER_DOCUMENTS
            and rerouted_rows == replay_rows
            and rerouted_calls > 0
            and compared == replay_rows * HIDDEN
            and 0 <= mismatched <= compared
            and 0 <= mismatched_calls <= rerouted_calls
        ):
            raise ValueError(f"layer {layer}: partial replay counters do not close")
    elif any(
        value != 0
        for value in (
            replay_rows,
        ...
        raise ValueError(f"layer {layer}: disabled partial replay is nonempty")
    if (
    ...
    manifest = {
        ...
        **applied_stats,
        ...
        "partial_replay": dict(partial_replay),
        "files": files,
    }
...
def validate_capture_runtime_record(
    ...
    *,
    ...
    optional_keys = {"progressive_w4a8"}
    if not (set(expected) | variable_keys).issubset(value) or (
        set(value) - (set(expected) | variable_keys | optional_keys)
    ):
    ...
    progressive_evidence = value.get("progressive_w4a8")
    if progressive_evidence is not None:
        if (
            not isinstance(progressive_evidence, dict)
            or progressive_evidence.get("schema")
            != "glm52-sqg-progressive-w4a8-validation-v1"
            or progressive_evidence.get("activation_endpoint") != "full-w4a8"
            or progressive_evidence.get("allow_a16_fallback") is not False
            or progressive_evidence.get("construction_only") is not True
        ):
            raise ValueError("progressive W4A8 activation-generator evidence differs")
        runtime_environment["B12X_MOE_FORCE_A16"] = "0"
    worker_expected = {
        "tensor_parallel_size": 4,
        "pipeline_parallel_size": 1,
        "data_parallel_size": 1,
        ...
        "exl3_environment": runtime_environment,
...
def validate_capture(capture_dir: str | Path, *, verify_hashes: bool = True) -> dict:
    ...
    if manifest.get("schema") == "glm52-r10-sqg-capture-view-v1":
        from .r10_capture_view import validate_r10_sqg_capture_view

        return validate_r10_sqg_capture_view(
            root,
            verify_hashes=verify_hashes,
            selected_layers=SELECTED_LAYERS,
            expected_split=EXPECTED_SPLIT,
            load_document_plan=load_document_plan,
            sha256_file=sha256_file,
        )
    if manifest.get("schema") == "glm52-pp8-sqg-capture-view-v1":
        from .pp8_capture_view import validate_pp8_sqg_capture_view

        return validate_pp8_sqg_capture_view(
            root,
            verify_hashes=verify_hashes,
            selected_layers=SELECTED_LAYERS,
            expected_split=EXPECTED_SPLIT,
