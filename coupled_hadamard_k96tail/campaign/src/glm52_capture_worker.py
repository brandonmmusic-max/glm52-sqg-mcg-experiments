...
def glm_reference_route_scores(router_logits, correction_bias, torch):
    ...
    # The live fused router evaluates sigmoid through tanhf.  Using the same
    # algebra here avoids a false top-k disagreement when two experts straddle
    # the cutoff by only a few float32 ulps.
    unbiased_scores = 0.5 * torch.tanh(0.5 * logits) + 0.5
...
class FreshSQGCaptureWorkerExtension:
    ...
    def _fresh_runtime_contract(self) -> dict:
        ...
        if rank not in (0, 1, 2, 3):
            raise RuntimeError(f"fresh-SQG capture requires TP rank 0..3, got {rank}")
        expected_environment = expected_runtime_env()
        observed_environment = {
            key: os.environ.get(key) for key in expected_environment
        }
        triton_cache_base = expected_environment["TRITON_CACHE_DIR"]
        expected_environment["TRITON_CACHE_DIR"] = (
            f"{triton_cache_base}/rank-{rank:02d}"
        )
        if observed_environment != expected_environment:
            raise RuntimeError(
                "fresh-SQG capture rank-private environment differs: "
                f"{observed_environment} != {expected_environment}"
            )

        # Every worker must resolve a distinct rank-private Triton directory to
        # prevent concurrent compiler writers.  After checking the concrete
        # path above, normalize it to the sealed parent so the common runtime
        # contract remains byte-identical across ranks.
        normalized_environment = dict(observed_environment)
        normalized_environment["TRITON_CACHE_DIR"] = triton_cache_base
        observed = {
            ...
            "exl3_environment": normalized_environment,
            "triton_cache_resolution": "rank-private path validated before normalization",
        }
        expected = {
            ...
            "exl3_environment": expected_runtime_env(),
            "triton_cache_resolution": "rank-private path validated before normalization",
        }
    ...
    def _fresh_fused_layer_audit(self) -> dict:
        ...
        progressive_path = os.environ.get("FRESH_SQG_PROGRESSIVE_W4A8_MANIFEST")
        progressive: dict | None = None
        if progressive_path:
            path = Path(progressive_path)
            if not path.is_file() or path.is_symlink():
                raise RuntimeError("progressive W4A8 construction manifest is unsafe")
            progressive = json.loads(path.read_text(encoding="utf-8"))
            if (
                not isinstance(progressive, dict)
                or progressive.get("schema")
                != "glm52-sqg-progressive-w4a8-construction-v1"
                or progressive.get("complete") is not True
                or progressive.get("allow_a16_fallback") is not False
            ):
                raise RuntimeError("progressive W4A8 construction manifest differs")
        ...
        fused: list[int] = []
        observed_w4a8: list[int] = []
        observed_mcg: list[int] = []
        for name, module in modules.items():
            match = pattern.search(name)
            if match is None:
                continue
            ...
            if progressive is not None:
                is_w4a8 = hasattr(routed_experts, "exl3_glm_sqg_w4a8_weights")
                mcg_count = sum(
                    len(getattr(routed_experts, f"{group}_mcg").exl3_tensors)
                    for group in ("w13", "w2")
                )
                sqg_count = sum(
                    len(getattr(routed_experts, f"{group}_sqg").exl3_tensors)
                    for group in ("w13", "w2")
                )
                if is_w4a8:
                    if sqg_count != 768 or mcg_count != 0:
                        raise RuntimeError(
                            f"layer {layer}: W4A8 marker census differs "
                            f"(SQG={sqg_count}, MCG={mcg_count})"
                        )
                    observed_w4a8.append(layer)
                else:
                    if mcg_count != 768 or sqg_count != 0:
                        raise RuntimeError(
                            f"layer {layer}: MCG marker census differs "
                            f"(SQG={sqg_count}, MCG={mcg_count})"
                        )
                    observed_mcg.append(layer)
        ...
        fused = sorted(fused)
        if progressive is not None:
            encoded = list(progressive.get("encoded_layers", ()))
            unchanged = list(progressive.get("unchanged_mcg_layers", ()))
            if sorted(observed_w4a8) != encoded or sorted(observed_mcg) != unchanged:
                raise RuntimeError(
                    "post-load progressive codebook population differs: "
                    f"W4A8={sorted(observed_w4a8)} MCG={sorted(observed_mcg)}"
                )
            audit = {
                "schema": "glm52-progressive-w4a8-layer-execution-audit-v1",
                "routed_layers": list(ROUTED_LAYERS),
                "encoded_w4a8_layers": encoded,
                "unchanged_mcg_layers": unchanged,
                "w4a8_count": len(encoded),
                "mcg_count": len(unchanged),
                "selected_layer_modes": {
                    str(layer): "native_full_w4a8" if layer in encoded else "mcg"
                ...
                "construction_manifest_sha256": sha256_file(Path(progressive_path)),
            }
            try:
    ...
    def _fresh_router_audit(
        ...
        *,
        ...
        expected_class_sources = {
            role: dict(record) for role, record in RUNTIME_CLASS_SOURCES.items()
        }
        if os.environ.get("FRESH_SQG_PROGRESSIVE_W4A8_MANIFEST"):
            expected_class_sources["quant_method"]["sha256"] = os.environ[
                "FRESH_SQG_PROGRESSIVE_EXL3_SHA256"
            ]
        if class_sources != expected_class_sources:
            raise RuntimeError(
                f"layer {layer}: live runner/router class sources differ: "
                f"{class_sources} != {expected_class_sources}"
            )
        audit = {
            "layer": layer,
            "execution_mode": (
                "progressive_native_full_w4a8"
                if os.environ.get("FRESH_SQG_PROGRESSIVE_W4A8_MANIFEST")
                else "sealed_all_mcg_teacher"
            ),
            "module_name": name,
    ...
    def fresh_sqg_capture_init(
        ...
        resume_documents = int(resume_documents)
        if resume_rows < 0 or resume_documents < 0 or (
            (resume_rows == 0) != (resume_documents == 0)
        ):
            raise RuntimeError("fresh-SQG partial-resume counters are invalid")
        ...
        self._fresh_handles = {}
        self._fresh_replay_handles = {}
        self._fresh_replay_prefetched = {}
        self._fresh_resume_rows = resume_rows
        self._fresh_resume_documents = resume_documents
        self._fresh_hashes = {}
        self._fresh_bytes = {}
        ...
        self._fresh_router_owners = {}
        self._fresh_original_selects = {}
        self._fresh_router_audits = {}
        self._fresh_partial_replay = {
            layer: {
                "schema": "fresh-sqg-retained-hidden-router-replay-v1",
                "enabled": bool(resume_rows),
                "resume_rows": resume_rows,
                "resume_documents": resume_documents,
                "retained_hidden_rerouted_rows": 0,
                "retained_hidden_rerouted_calls": 0,
                "live_hidden_compared_elements": 0,
                "live_hidden_mismatched_elements": 0,
                "live_hidden_mismatched_calls": 0,
                "route_source": (
                    "owner.gate(retained_hidden) followed by the original bound "
        ...
        for layer, (name, owner, module, router) in routers.items():
            if not callable(getattr(owner, "gate", None)):
                raise RuntimeError(f"layer {layer}: owning GLM MoE gate is absent")
            self._fresh_router_owners[layer] = owner
            # Save the bound method before installing the post-select wrapper.
            # Partial replay uses it to route retained hidden-state bytes without
            # recursively invoking capture.
            self._fresh_original_selects[layer] = router.select_experts
            ...
            for filename in FILE_ABI:
                ...
                key = (layer, filename)
                if resume_rows:
                    expected_bytes = resume_rows * int(
                        FILE_ABI[filename]["bytes_per_row"]
                    )
                    if not partial.is_file() or partial.is_symlink():
                        raise RuntimeError(
                            f"layer {layer}: retained partial {filename} differs"
                        )
                    retained_bytes = partial.stat().st_size
                    if retained_bytes not in (0, expected_bytes):
                        raise RuntimeError(
                            f"layer {layer}: retained partial {filename} differs"
                        )
                    if retained_bytes:
                        self._fresh_replay_handles[key] = partial.open(
                            "rb", buffering=16 << 20
                        )
                    self._fresh_handles[key] = partial.open(
                        "ab", buffering=16 << 20
                    )
                else:
                    if partial.exists():
                        raise RuntimeError(
                            f"layer {layer}: refusing to overwrite existing "
                            f"{filename} payload"
                        )
                    self._fresh_handles[key] = partial.open(
                        "xb", buffering=16 << 20
    ...
    def _fresh_write(self, layer: int, filename: str, value: bytes) -> None:
        ...
        if active is None:
            raise RuntimeError("capture payload write has no active document")
        if bool(active.get("replay")):
            prefetched = self._fresh_replay_prefetched.pop(key, None)
            replay = self._fresh_replay_handles.get(key)
            if prefetched is not None:
                if prefetched != value:
                    raise RuntimeError(
                        f"layer {layer}: retained prefetched replay differs for "
                        f"{filename}"
                    )
            elif replay is None:
                # This payload's userspace buffer was lost when the prior
                # worker failed.  Regenerate it while replaying the retained
                # document prefix; other nonempty payloads still provide an
                # exact byte-for-byte determinism check.
                self._fresh_handles[key].write(value)
            else:
                expected = replay.read(len(value))
                if expected != value:
                    raise RuntimeError(
                        f"layer {layer}: retained partial replay differs for {filename}"
                    )
        else:
            self._fresh_handles[key].write(value)
    ...
    def _fresh_recompute_retained_route(
        self,
        layer: int,
        live_hidden,
        torch,
    ):
        """Route the exact retained hidden prefix through frozen live weights.

        Eager FP8-KV execution is not bitwise stable across engine boots.  A
        partial capture therefore cannot pair retained hidden states with route
        data generated from a new boot.  During prefix replay this method reads
        the retained BF16 bytes once, reconstructs that exact tensor on device,
        and reruns the owning GLM gate plus the original router implementation.
        """

        key = (layer, "hidden.bf16.bin")
        replay = self._fresh_replay_handles.get(key)
        if replay is None:
            return None
        rows = int(live_hidden.shape[0])
        size = rows * HIDDEN * 2
        expected = replay.read(size)
        if len(expected) != size:
            raise RuntimeError(
                f"layer {layer}: retained hidden replay ended inside a document"
            )
        if key in self._fresh_replay_prefetched:
            raise RuntimeError(f"layer {layer}: retained hidden replay was prefetched twice")
        self._fresh_replay_prefetched[key] = expected

        retained_words = np.frombuffer(expected, dtype="<u2").copy()
        live_words = (
            live_hidden.detach()
            .contiguous()
            .cpu()
        ...
        mismatch = int(np.count_nonzero(live_words != retained_words))
        audit = self._fresh_partial_replay[layer]
        audit["live_hidden_compared_elements"] += int(retained_words.size)
        audit["live_hidden_mismatched_elements"] += mismatch
        audit["live_hidden_mismatched_calls"] += int(mismatch != 0)

        retained_cpu = torch.from_numpy(retained_words).view(torch.bfloat16)
        retained_hidden = retained_cpu.reshape(rows, HIDDEN).to(
            device=live_hidden.device
        )
        owner = self._fresh_router_owners[layer]
        original_select = self._fresh_original_selects[layer]
        with torch.no_grad():
            gate_result = owner.gate(retained_hidden)
            if not isinstance(gate_result, tuple) or len(gate_result) != 2:
                raise RuntimeError(f"layer {layer}: owner.gate return schema changed")
            retained_logits = gate_result[0]
            selected = original_select(
                hidden_states=retained_hidden,
                router_logits=retained_logits,
            )
        if not isinstance(selected, tuple) or len(selected) != 2:
            raise RuntimeError(
                f"layer {layer}: original router.select_experts return schema changed"
            )
        audit["retained_hidden_rerouted_rows"] += rows
        audit["retained_hidden_rerouted_calls"] += 1
        return retained_hidden, retained_logits, selected[0], selected[1]
    ...
    def _fresh_capture_router_result(
        ...
        if bool(active.get("replay")):
            retained = self._fresh_recompute_retained_route(layer, hidden, torch)
            if retained is not None:
                hidden, router_logits, topk_weights, topk_ids = retained
        if (
    ...
    def fresh_sqg_begin_document(
        ...
        self._fresh_active = {
            ...
            "replay": epoch < self._fresh_resume_documents,
        }
    ...
    def fresh_sqg_end_document(self) -> dict:
        ...
        if self._fresh_doc_count == self._fresh_resume_documents:
            if min(self._fresh_counts.values()) != self._fresh_resume_rows:
                raise RuntimeError("partial replay row total differs")
            for key, handle in self._fresh_replay_handles.items():
                if handle.read(1) != b"":
                    raise RuntimeError(
                        f"partial replay did not consume retained payload {key}"
                    )
                handle.close()
        self._fresh_active = None
        return {
    ...
    def fresh_sqg_capture_abort(self) -> dict:
        ...
        for handle in self._fresh_handles.values():
            if not handle.closed:
                handle.flush()
                handle.close()
        for handle in getattr(self, "_fresh_replay_handles", {}).values():
            if not handle.closed:
                handle.close()
        return {
            "rank": rank,
    ...
    def fresh_sqg_capture_finalize(self) -> dict:
        ...
        for handle in self._fresh_replay_handles.values():
            if not handle.closed:
                raise RuntimeError("partial replay handle remained open at finalize")
        ...
        for layer in SELECTED_LAYERS:
            ...
            layers[str(layer)] = {
                ...
                "partial_replay": self._fresh_partial_replay[layer],
                "files": files,
            }
        return {
            "rank": rank,
