...
RUNTIME_FILES = {
    ...
    "/opt/venv/lib/python3.12/site-packages/vllm/v1/executor/"
    "multiproc_executor.py": (
        "f664eac1eba4cc6e52a01b6bc95a04ccd58a21051964d20e628a5102cc72f1fc"
    ),
...
RUNTIME_ENV = {
    "FRESH_SQG_RANK_PRIVATE_TRITON": "1",
...
def expected_runtime_env(environ: dict[str, str] | None = None) -> dict[str, str]:
    """Return the exact all-MCG or construction-only W4A8 environment."""

    environ = dict(os.environ) if environ is None else dict(environ)
    expected = dict(RUNTIME_ENV)
    if environ.get("FRESH_SQG_PROGRESSIVE_W4A8_MANIFEST"):
        if environ.get("VLLM_GLM_SQG_W4A8_ALLOW_PARTIAL_CONSTRUCTION") != "1":
            raise RuntimeError("progressive W4A8 runtime opt-in is absent")
        expected["B12X_MOE_FORCE_A16"] = "0"
    return expected
...
def _python_tree_sha256(root: Path) -> str:
    ...
    for path in sorted(root.rglob("*.py")):
        if path.is_symlink() or not path.is_file():
            raise RuntimeError(f"unsafe Python runtime file: {path}")
        records.append(
            {
    ...
    if not records:
        raise RuntimeError(f"Python runtime tree is empty: {root}")
...
def validate_capture_runtime(
    *,
    ...
    environ = dict(os.environ) if environ is None else dict(environ)
    runtime_files = dict(RUNTIME_FILES) if runtime_files is None else dict(runtime_files)
    ...
    expected_env = expected_runtime_env(environ)
    observed_env = {key: environ.get(key) for key in expected_env}
    if observed_env != expected_env:
        raise RuntimeError(
            f"EXL3 capture environment differs: {observed_env} != {expected_env}"
        )
    ...
    progressive = bool(environ.get("FRESH_SQG_PROGRESSIVE_W4A8_MANIFEST"))
    if progressive:
        expected_exl3 = environ.get("FRESH_SQG_PROGRESSIVE_EXL3_SHA256")
        if not expected_exl3:
            raise RuntimeError("progressive EXL3 runtime hash is absent")
        runtime_files[
            "/opt/venv/lib/python3.12/site-packages/vllm/model_executor/layers/"
            "quantization/exl3.py"
        ] = expected_exl3
    ...
    if progressive:
        b12x_root = Path("/opt/venv/lib/python3.12/site-packages/b12x")
        observed_tree = _python_tree_sha256(b12x_root)
        expected_tree = environ.get("FRESH_SQG_PROGRESSIVE_B12X_TREE_SHA256")
        if observed_tree != expected_tree:
            raise RuntimeError(
                f"progressive B12X runtime tree differs: {observed_tree} != "
                f"{expected_tree}"
            )
        files[str(b12x_root)] = {
            "sha256": observed_tree,
            "kind": "canonical_python_tree",
        }
