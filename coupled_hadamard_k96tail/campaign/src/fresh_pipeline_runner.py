...
from .glm52_bf16_source import (
    BF16ExpertSource,
    open_sealed_bf16_source,
...
def _validate_source_seal(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or seal.get("target_tensor_count") != len(SELECTED_LAYERS) * 768
...
def open_layer_runtime(
    ...
    *,
    ...
    expected_visible_gpu = resolved_visible_gpu(layer)
    if (
    ...
    source_seal = _validate_source_seal(paths.source_seal)
    # Payload hashes were sealed by the immutable preflight. Paid workers bind
    # its exact inode/size/mtime domain and cache the validated provider in the
    # persistent slot; every reuse still performs cheap file-identity checks.
    source = open_sealed_bf16_source(
        source_seal=source_seal,
        shard_file_identity=preflight["source_seal"]["shard_file_identity"],
        cache_namespace=os.environ.get("B300_WEIGHT_CACHE_NAMESPACE"),
    )
    # The paid PP8 capture is sealed by exact source inode/size and canonical
    # row-order evidence.  When its preflight deliberately skipped payload
    # hashes, do not turn every atomic worker into a redundant 12-GiB reader.
    # The capture-view validator still rechecks regular-file identity, byte
    # geometry, document vectors, and routing closure on every open.
    capture = LayerCaptureView(
        paths.capture_dir,
        layer,
        validate=True,
        verify_hashes=bool(preflight["capture"]["hashes_verified"]),
    )
...
def resolved_visible_gpu(layer: int) -> str:
    """Resolve sealed four-GPU placement or the explicit paid-node override."""

    value = os.environ.get("FRESH_SQG_EXPECTED_VISIBLE_GPU_OVERRIDE")
    if value is None:
        value = str(SELECTED_LAYERS.index(layer))
    if value not in {str(index) for index in range(8)}:
        raise ValueError("physical GPU override must be one of 0..7")
    return value
...
def _load_h13(runtime: LayerRuntime) -> tuple[DenseHessian, dict[str, Any]]:
    ...
    with safe_open(path, framework="pt", device=runtime.device) as handle:
        matrix = handle.get_tensor("h13").float().contiguous()
    if matrix.device.type != "cuda":
        raise RuntimeError("production H13 was not restored directly on CUDA")
...
def _load_scale_evidence(runtime: LayerRuntime) -> ProfileScaleEvidence:
    ...
    if any(value.device.type != "cuda" for value in values.values()):
        raise RuntimeError("production profile scales were not restored on CUDA")
