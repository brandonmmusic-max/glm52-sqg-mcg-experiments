...
R10_PLAN_SCHEMA = "glm52-r10-sqg-document-plan-v1"
R10_PLAN_FILE_SHA256 = (
    "2938810d626c48557ff2439cbacabd748ca873094a115beeff09ada270a4bae1"
)
R10_PLAN_FINGERPRINT = (
    "af6e0cdf2eacdb4dd7ce4a1d53d8101528e98f688a0932800f8a496365e7844b"
)
...
if _PLAN_CONTRACT_PATH:
    ...
    _contract_schema = _plan_contract.get("schema")
    _is_r10_contract = _contract_schema == R10_PLAN_SCHEMA
    _old_contract_schemas = {
        "glm52-fresh-sqg-document-plan-v1",
        "glm52-fresh-sqg-contiguous-document-plan-v1",
    }
    _provenance_valid = (
        _plan_contract.get("corpus_sha256") == CORPUS_SHA256
        and (
            (
                not _is_r10_contract
                and _contract_schema in _old_contract_schemas
                and _plan_contract.get("owner_manifest_sha256")
                == OWNER_MANIFEST_SHA256
                and _plan_contract.get("owner_capture_fingerprint")
                == OWNER_CAPTURE_FINGERPRINT
            )
            or (
                _is_r10_contract
                ...
                and _plan_contract.get("owner_capture_fingerprint")
                == R10_PLAN_FINGERPRINT
                and _plan_contract.get("r10_corpus_plan_file_sha256")
                == R10_PLAN_FILE_SHA256
                and _plan_contract.get("r10_corpus_plan_sha256")
                == R10_PLAN_FINGERPRINT
            )
        )
    )
    if not _provenance_valid:
        raise ValueError("calibration plan is not derived from the sealed owner plan")
    if _is_r10_contract:
        # Retain the legacy field names in validate_document_plan while binding
        # them to the authoritative R10 source-plan identities.
        OWNER_MANIFEST_SHA256 = R10_PLAN_FILE_SHA256
        OWNER_CAPTURE_FINGERPRINT = R10_PLAN_FINGERPRINT
    ...
    if (
        ...
        raise ValueError("calibration plan split/documents are invalid")
    ...
    if OWNER_DOCUMENTS != len(_documents) or OWNER_TOKENS != sum(
        ...
        raise ValueError("calibration plan totals do not close")
    if _contract_schema == "glm52-fresh-sqg-document-plan-v1" and (
        OWNER_DOCUMENTS != 4_497
        or OWNER_TOKENS != 1_050_468
        or EXPECTED_SPLIT
        != {
            "fit": {"documents": 2_638, "tokens": 601_343},
            "selection": {"documents": 892, "tokens": 219_650},
        ...
        raise ValueError("full owner calibration plan totals differ")
