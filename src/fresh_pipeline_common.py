...
EXPECTED_K3_PER_LAYER = 384
EXPECTED_K4_PER_LAYER = 384
EXPECTED_BIT_UNITS_PER_LAYER = 2_688
...
class LayerBitBudget:
    ...
    def __post_init__(self) -> None:
        ...
        if (values.count(3), values.count(4), sum(values)) != (
            ...
            raise ValueError(f"layer {self.layer}: exact 384/384 bit budget drift")
...
def load_bit_contract(path: str | Path) -> dict[int, LayerBitBudget]:
    ...
    if totals != {"K3": 1536, "K4": 1536, "layers": 4, "tensors": 3072}:
        raise ValueError("four-layer bit-allocation totals differ")
    return budgets
