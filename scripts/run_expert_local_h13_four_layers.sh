...
[[ $# -eq 1 ]] || die "usage: $0 /absolute/fresh-output-root"
...
BASELINE_ROOT="/home/brandonmusic/KLC_SANDBOXES/fresh-sqg-encode-absfix-r2"
...
run_worker() {
  ...
  docker run --rm --name "$name" \
    --network none --gpus all --shm-size 16g --cpus 3 \
...
for index in 0 1 2 3; do
  ...
  for shard in 0 1 2 3; do
    ...
    name="glm52-sqg-h13e-l${layer}-p${shard}"
    names+=("$name")
...
[[ "$failed" -eq 0 ]] || die "one or more expert-local H13 workers failed"

docker run --rm --network none \
  --mount "type=bind,src=$OUTPUT_ROOT,dst=/candidate" \
  alpine:3.20 chmod -R a+rX /candidate

printf 'Expert-local H13 four-layer encode complete: %s\n' "$OUTPUT_ROOT"
